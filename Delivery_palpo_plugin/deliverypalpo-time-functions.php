 ...
 */
function palpodelivery_get_current_time() {
	$gmt = false;
 ...
 */
function palpodelivery_get_cutoff_timestamp( $cutoff_day = 'same_day' ) {
	$current_time  = palpodelivery_get_current_time();
	$current_day   = gmdate( 'd', $current_time );
	$current_month = gmdate( 'm', $current_time );
	$current_year  = gmdate( 'Y', $current_time );

	$cutoff_hours = get_option( 'palpodelivery_disable_same_day_delivery_after_hours' );
	$cutoff_mins  = get_option( 'palpodelivery_disable_same_day_delivery_after_minutes' );

	if ( 'next_day' === $cutoff_day ) {
		$cutoff_hours = get_option( 'palpodelivery_disable_next_day_delivery_after_hours' );
		$cutoff_mins  = get_option( 'palpodelivery_disable_next_day_delivery_after_minutes' );
	}

	if ( 'on' === get_option( 'palpodelivery_enable_day_wise_settings' ) ) {
		$current_weekday  = 'palpodelivery_weekday_' . gmdate( 'w', $current_time );
		...
		if ( '' !== $advance_settings && '{}' !== $advance_settings && '[]' !== $advance_settings ) {
			foreach ( $advance_settings as $ak => $av ) {
				if ( $current_weekday === $av['palpodelivery_weekdays'] ) {
					if ( 'same_day' === $cutoff_day && '' !== $av['palpodelivery_disable_same_day_delivery_after_hours'] ) {
						$cut_off_time = explode( ':', $av['palpodelivery_disable_same_day_delivery_after_hours'] );
						$cutoff_hours = $cut_off_time[0];
						$cutoff_mins  = $cut_off_time[1];
					} elseif ( 'next_day' === $cutoff_day && '' !== $av['palpodelivery_disable_next_day_delivery_after_hours'] ) {
	...
	return apply_filters( 'palpodelivery_modify_cutoff_timestamp', $cut_off_timestamp, $cutoff_day );
}

/**
 * Get the minimum delivery time in seconds for general settings.
 */
function palpodelivery_get_minimum_delivery_time() {
	$current_time          = palpodelivery_get_current_time();
	$minimum_delivery_time = '' !== get_option( 'palpodelivery_minimumOrderDays' ) ? get_option( 'palpodelivery_minimumOrderDays' ) * 60 * 60 : 0;
			...
			foreach ( $advance_settings as $ak => $av ) {
				if ( $current_weekday === $av['palpodelivery_weekdays'] ) {
					if ( '' !== $av['palpodelivery_minimumOrderDays'] ) {
						$minimum_delivery_time = $av['palpodelivery_minimumOrderDays'] * 60 * 60;
					}
				}
			}
		}
	...
	return apply_filters( 'palpodelivery_modify_minimum_delivery_time', $minimum_delivery_time );
}

/**
 * Get the minimum delivery time for custom settings.
 */
function palpodelivery_get_minimum_delivery_time_custom() {
	if ( 'on' === get_option( 'palpodelivery_enable_shipping_based_delivery' ) ) {
		...
		if ( isset( $_POST['palpodelivery_location'] ) ) { //phpcs:ignore
			$location = $_POST['palpodelivery_location']; //phpcs:ignore
		}
		...
		foreach ( $results as $key => $value ) {
			...
			'palpodelivery_locations' === $shipping_settings['delivery_settings_based_on'][0] ) {
				if ( in_array( $location, $shipping_settings['palpodelivery_locations'], true ) ) {
				...
				if ( has_filter( 'palpodelivery_get_shipping_method' ) ) {
					$shipping_methods_values               = apply_filters( 'palpodelivery_get_shipping_method', $custom_settings, $_POST, $shipping_settings['shipping_methods'], $shipping_method ); //phpcs:ignore
	...
	return apply_filters( 'palpodelivery_modify_minimum_delivery_time_custom', $minimum_delivery_time, $custom_settings );
}

/**
 * Get the highest minimum delivery time for 2 product categories or shipping classes.
 */
function palpodelivery_get_higher_minimum_delivery_time() {
	$minimum_delivery_time = wp_cache_get( 'palpodelivery_get_higher_minimum_delivery_time' );
	...
	if ( false === $minimum_delivery_time ) {
		...
		if ( 'on' === $palpodelivery_shipping_based_delivery ) {
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
					...
					if ( '' !== $terms ) {
						foreach ( $terms as $term => $val ) {
							$id = palpodelivery_common::get_base_product_category( $val->term_id );
					...
					'product_categories' === $shipping_settings['delivery_settings_based_on'][0] ) {
						if ( isset( $shipping_settings['product_categories'] ) && ! isset( $shipping_settings['shipping_methods_for_categories'] ) ) {
							...
							foreach ( $terms_id as $term => $val ) {
								$cat_slug = palpodelivery_common::ordd_get_cat_slug( $val );
			...
			wp_cache_set( 'palpodelivery_get_higher_minimum_delivery_time', $minimum_delivery_time );
		}
	}
 ...
 */
function palpodelivery_get_highest_same_day() {
	$results        = palpodelivery_common::palpodelivery_get_shipping_settings();
	$same_day       = array();
	$same_day_hours = 0;
	$same_day_min   = 00;
	...
	$palpodelivery_shipping_based_delivery = get_option( 'palpodelivery_enable_shipping_based_delivery' );
	if ( 'on' === $palpodelivery_shipping_based_delivery && is_array( $results ) && count( $results ) > 0 ) {
		foreach ( $results as $key => $value ) {
			$shipping_settings = get_option( $value->option_name );
 ...
 */
function palpodelivery_get_highest_next_day() {
	$next_day       = array();
	$next_day_hours = 0;
	$next_day_min   = 00;
