...
class palpodelivery_common {
	...
	public function __construct() {
		if ( true === is_admin() ) {
			add_filter( 'ts_tracker_data',                          array( &$this, 'palpodelivery_ts_add_plugin_tracking_data' ), 10, 1 );
			add_filter( 'ts_tracker_opt_out_data',                  array( &$this, 'palpodelivery_get_data_for_opt_out' ), 10, 1 );
			add_filter( 'ts_deativate_plugin_questions',            array( &$this, 'palpodelivery_deactivate_add_questions' ), 10, 1 );
		}
	}
	 ...
	 */
	public function palpodelivery_deactivate_add_questions () {
		$ts_deactivate = array(
			0 => array(
				'id'                => 4,
     ...
     * @since 6.8
     */
	public function palpodelivery_get_data_for_opt_out( $params ) {
	...
	private static function ts_get_plugin_version() {
		global $palpodelivery_version;
		return $palpodelivery_version;
	}

	/**
	 * Get all plugin options starting with palpodelivery_ prefix.
	 *
	...
	private static function ts_get_all_plugin_options_values() {
		global $wpdb;
		$palpodelivery_custom_count = 0;
		$results = palpodelivery_common::palpodelivery_get_shipping_settings();

		if( isset( $results[0] ) ) {
			$palpodelivery_custom_count = $results[0]->custom_settings_count;
		}

		return array(
			'enable_delivery'                       => get_option( 'palpodelivery_enable_delivery_date' ),
			'delivery_options'                      => get_option( 'palpodelivery_delivery_checkout_options' ),
			'weekday_wise_settings'                 => get_option( 'palpodelivery_enable_day_wise_settings' ),
			'date_mandatory'                        => get_option( 'palpodelivery_date_field_mandatory' ),
			'shipping_days'                         => get_option( 'palpodelivery_enable_shipping_days' ),
			'specific_delivery_dates'               => get_option( 'palpodelivery_enable_specific_delivery_dates' ),
			'delivery_time'                         => get_option( 'palpodelivery_enable_delivery_time' ),
			'same_day_delivery'                     => get_option( 'palpodelivery_enable_same_day_delivery' ),
			'next_day_delivery'                     => get_option( 'palpodelivery_enable_next_day_delivery' ),
			'time_slot'                             => get_option( 'palpodelivery_enable_time_slot' ),
			'time_slot_mandatory'                   => get_option( 'palpodelivery_time_slot_mandatory' ),
			'populate_first_time_slot'              => get_option( 'palpodelivery_auto_populate_first_available_time_slot' ),
			'populate_first_delivery_date'          => get_option( 'palpodelivery_enable_autofill_of_delivery_date' ),
			'no_fields_for'                         => array( 'virtual_product'  => get_option( 'palpodelivery_no_fields_for_virtual_product' ),         'featured_product' => get_option( 'palpodelivery_no_fields_for_featured_product' ) ),
			'edit_date_for_customers'               => get_option( 'palpodelivery_allow_customers_to_edit_date' ),
			'shipping_multiple_address'             => get_option( 'palpodelivery_shipping_multiple_address_compatibility' ),
			'amazon_payments_advanced_gateway'      => get_option( 'palpodelivery_amazon_payments_advanced_gateway_compatibility' ),
			'woocommerce_subscriptions'             => get_option( 'palpodelivery_woocommerce_subscriptions_compatibility' ),
			'woocommerce_subscriptions_auto_update' => get_option( 'palpodelivery_woocommerce_subscriptions_auto_update' ),
			'custom_delivery'                       => get_option( 'palpodelivery_enable_shipping_based_delivery' ),
			'custom_delivery_count'                 => $palpodelivery_custom_count,
			'calendar_sync'                         => get_option( 'palpodelivery_calendar_sync_integration_mode' )
		 );
	}
	...
	private static function ts_get_order_counts() {
		global $wpdb;
		$order_count = 0;
		$palpodelivery_query = "SELECT count(ID) AS delivery_orders_count FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ('wc-cancelled', 'wc-refunded', 'trash', 'wc-failed' ) AND ID IN ( SELECT post_id FROM `" . $wpdb->prefix . "postmeta` WHERE meta_key IN ( %s, %s ) )";

		$results = $wpdb->get_results( $wpdb->prepare( $palpodelivery_query, '_palpodelivery_timestamp', get_option( 'palpodelivery_delivery_date_field_label' ) ) );
		if( isset( $results[0] ) ) {
    ...
    public function palpodelivery_ts_add_plugin_tracking_data( $data ) {
    	if ( isset( $_GET[ 'palpodelivery_tracker_optin' ] ) && isset( $_GET[ 'palpodelivery_tracker_nonce' ] ) && wp_verify_nonce( $_GET[ 'palpodelivery_tracker_nonce' ], 'palpodelivery_tracker_optin' ) ) {
	        ...
	        $plugin_data[ 'palpodelivery_plugin_version' ]    = self::ts_get_plugin_version();
	        $plugin_data[ 'license_key_info' ]        = self::ts_get_plugin_license_key();
	        $plugin_data[ 'palpodelivery_allow_tracking' ]    = get_option ( 'palpodelivery_allow_tracking' );
     ...
     *
     * @globals array $palpodelivery_languages Languages array
     * @globals array $palpodelivery_languages_locale Languages locale array
     *
	...
	public static function delivery_date_language( $delivery_date_formatted, $delivery_date_timestamp ) {
		...
		global $palpodelivery_languages, $palpodelivery_languages_locale, $palpodelivery_date_formats, $wp_locale;
		require_once ABSPATH . 'wp-admin/includes/translation-install.php';
		require_once ABSPATH . 'wp-admin/includes/file.php';

		$date_language = get_option( 'palpodelivery_language_selected' );
		if( $delivery_date_timestamp != '' ) {

            if( $date_language != 'en-GB' ) {
				$locale_format = $palpodelivery_languages[ $date_language ];
				$lang = explode( '.', $palpodelivery_languages_locale[ $locale_format ][0] );
				...
				$date_format = get_option( 'palpodelivery_delivery_date_format' );

				$time_settings = date( "H:i", $delivery_date_timestamp );

				$delivery_date_formatted = date_i18n( $palpodelivery_date_formats[ $date_format ] ,$delivery_date_timestamp );

				if ( $time_settings != '00:00' && $time_settings != '00:01'  ) {
					$time_format_to_show 		= palpodelivery_common::palpodelivery_get_time_format();
					$delivery_date_formatted 	= date_i18n( $palpodelivery_date_formats[ $date_format ].' '.$time_format_to_show, $delivery_date_timestamp );
				}
	 ...
	 *
	 * @globals array $palpodelivery_date_formats Date formats array
	 *
	 * @param int $order_id Order ID
	 * @return string Delivery date for an order
	 * @since 2.6.3
	 */
	public static function palpodelivery_get_order_delivery_date( $order_id ) {

	    global $palpodelivery_date_formats;

	    $delivery_date_timestamp = get_post_meta( $order_id, '_palpodelivery_timestamp', true );
		...
		if ( $delivery_date_timestamp != "" ) {
			...
			$delivery_date_formatted = palpodelivery_common::delivery_date_language( $delivery_date_formatted, $delivery_date_timestamp );
		}
	 ...
	 * @since 2.6.3
	 */
	public static function palpodelivery_get_order_timeslot( $order_id, $is_subscription_parent_order = false ) {
        ...
        if ( 'shop_order' != $typenow ) {
	  		$location = palpodelivery_common::palpodelivery_get_order_location( $order_id );
	        $shipping_method = palpodelivery_common::palpodelivery_get_order_shipping_method( $order_id );
	        $product_category = palpodelivery_common::palpodelivery_get_cart_product_categories( $order_id );
	        $shipping_class = palpodelivery_common::palpodelivery_get_cart_shipping_classes( $order_id );

	        $field_label = palpodelivery_common::palpodelivery_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location );
        } else {
    	...
    	if ( isset( $data[ '_palpodelivery_time_slot' ] ) && '' != $data[ '_palpodelivery_time_slot' ] ) {
            $order_time_slot = $data[ '_palpodelivery_time_slot' ][ 0 ];
	 ...
	 * @since 8.7
	 */
	public static function palpodelivery_get_order_estimated_text_block( $order_id ) {
		global $palpodelivery_date_formats;
		$palpodelivery_estimated_shipping_timestamp = '';
		$palpodelivery_text_block = '';
 		$data = get_post_meta( $order_id );

        if( isset( $data[ 'palpodelivery_estimated_shipping_timestamp' ] ) ) {
            $palpodelivery_estimated_shipping_timestamp = $data[ 'palpodelivery_estimated_shipping_timestamp' ][ 0 ];

            if( '' != $palpodelivery_estimated_shipping_timestamp ) {
            	$shipping_date = date( $palpodelivery_date_formats[ get_option( 'palpodelivery_delivery_date_format') ], $palpodelivery_estimated_shipping_timestamp );

				if( 'on' == get_option( 'palpodelivery_enable_shipping_based_delivery' ) ) {
					$location           = palpodelivery_common::palpodelivery_get_order_location( $order_id );
					$shipping_method    = palpodelivery_common::palpodelivery_get_order_shipping_method( $order_id );
					$product_category   = palpodelivery_common::palpodelivery_get_cart_product_categories( $order_id );
					$shipping_class     = palpodelivery_common::palpodelivery_get_cart_shipping_classes( $order_id );
					...
					if( is_array( $results ) && count( $results ) > 0 ) {
						...
						foreach ( $results as $key => $value ) {
							...
							if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'palpodelivery_locations' ) {
								if( isset( $shipping_settings[ 'palpodelivery_locations' ] ) ) {
									$shipping_methods = $shipping_settings[ 'palpodelivery_locations' ];
								}
							...
							if( in_array( $shipping_method, $shipping_methods ) || in_array( $shipping_class, $shipping_methods ) || ( is_array( $shipping_method ) && count( array_intersect( $shipping_method, $shipping_methods ) ) ) ) {
								...
								$palpodelivery_min_between_days =  $shipping_settings[ 'palpodelivery_min_between_days' ];
								$palpodelivery_max_between_days =  $shipping_settings[ 'palpodelivery_max_between_days' ];
							}
						}
					}
				}else {
					$palpodelivery_min_between_days = get_option( 'palpodelivery_min_between_days' );
					$palpodelivery_max_between_days = get_option( 'palpodelivery_max_between_days' );

				}

            	$palpodelivery_text_block = __( 'Your Estimated shipping date for the order is ', 'order-delivery-date' ) . "<b>" . $shipping_date . '</b>. </br>The delivery will take approximately <b>' . $palpodelivery_min_between_days . "-" . $palpodelivery_max_between_days . '</b> working days from the shipping days.' ;
            }
        }
        return $palpodelivery_text_block;
	}
	 ...
	 * @since 2.8.4
	 */
	public static function palpodelivery_get_betweendays( $FromDate, $ToDate ) {
	    $Days[] = $FromDate;
	...
	public static function get_shipping_setting_option_key( $row_id ) {
	    if ( $row_id != '') {
	        ...
	        $option_key = get_option( 'palpodelivery_shipping_based_settings_option_key' );
	...
	public static function palpodelivery_time_settings_enable_for_custom_delivery() {
		...
		if ( false === $time_settings_for_shipping_method ) {
	        ...
	        if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' &&
	 ...
	 * @since 8.2
	 */
	public static function palpodelivery_time_slots_enable_for_custom_delivery() {
        global $wpdb;
		$results = palpodelivery_common::palpodelivery_get_shipping_settings();
        $shipping_settings =  array();
        $time_slots_for_shipping_method = 'no';
        if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' && count( $results ) > 0) {
            foreach ( $results as $key => $value ) {
                $shipping_settings = get_option( $value->option_name );
	...
	public static function palpodelivery_get_timestamp( $delivery_date, $date_format, $time_setting ) {
	    if( $delivery_date != '' ) {
	    	if ( 1 == date( "I" ) ) {
	 ...
	 * @globals array $palpodelivery_date_formats Date formats array
	 * @globals array $palpodelivery_languages Languages array
	 * @globals array $palpodelivery_weekdays Weekdays array
	 * @globals array $palpodelivery_shipping_days Shipping weekdays array
	 *
	 * @return string
	 * @since 1.0
	 */
	public static function load_hidden_fields( $called_from = '' ) {
	    global $wpdb, $palpodelivery_date_formats, $post, $woocommerce, $palpodelivery_languages, $palpodelivery_weekdays, $palpodelivery_shipping_days, $post;
	    ...
	    $var .= '<div id="palpodelivery_dynamic_hidden_vars"></div>';
	    $var .= '<input type="hidden" name="h_deliverydate" id="h_deliverydate" value="">';
	    $var .= '<input type="hidden" name="palpodelivery_unique_custom_settings" id="palpodelivery_unique_custom_settings" value="" >';
	    ...
	    if( is_admin() ) { // all days are to be enabled for the admin
	    	...
	    	$holidays_str = wp_cache_get( 'palpodelivery_general_delivery_date_holidays' );
	    	if( false == $holidays_str ) {
	    		$holidays_arr = array();
	    	    $holidays = get_option( 'palpodelivery_delivery_date_holidays' );
	    	    if ( $holidays != '' && $holidays != '{}' && $holidays != '[]' && $holidays != 'null' ) {
	                $holidays_arr = json_decode( get_option( 'palpodelivery_delivery_date_holidays' ) );
	    	    }
	    	    $holidays_str = "";
	    	    ...
	    	    wp_cache_set( 'palpodelivery_general_delivery_date_holidays', $holidays_str );
	    	}
	    }

	    $var .= '<input type="hidden" name="palpodelivery_delivery_date_holidays" id="palpodelivery_delivery_date_holidays" value=\'' . $holidays_str . '\'>';

	    //Lockout
	    $lockout_days_str = "";
	    if( !is_admin() ) {
	    	// todo: We can use the lockout date cache approch at other places also
	    	$lockout_days_str = wp_cache_get( 'palpodelivery_general_lockout_days_str' );
	    	if( false == $lockout_days_str ) {
		        $lockout_date_after_order = get_option( 'palpodelivery_lockout_date_after_orders' );
			    if ( get_option( 'palpodelivery_enable_time_slot' ) == 'on' ) {
		            $lockout_timeslots_arr = array();
		            $lockout_timeslots_days = get_option( 'palpodelivery_lockout_time_slot' );
		            if ( $lockout_timeslots_days != '' && $lockout_timeslots_days != '{}' && $lockout_timeslots_days != '[]' && $lockout_timeslots_days != 'null' ) {
		                $lockout_timeslots_arr = json_decode( get_option( 'palpodelivery_lockout_time_slot' ) );
		            }
		            $lockout = $previous_lockout = $previous_orders = 0;
		            $specific_dates = $delivery_days = $date = array();
		            $existing_timeslots_arr = json_decode( get_option( 'palpodelivery_delivery_time_slot_log' ) );
		            $blank_lockout_time_slot = 'no';
		            $all_lockout = 0;

		            foreach( $existing_timeslots_arr as $k => $v ) {
						$timeslot = $v->fh . ":" . $v->fm ."- ". $v->th . ":" . $v->tm;
		                    ...
		                    if ( is_array( $dd ) &&  count( $dd ) > 0 ) {
		                        foreach ( $dd as $dkey => $dval ) {
		                            if( $v->lockout != "" && $v->lockout != "0" ) {
		                                if( $v->tv == 'weekdays' ) {
		                                    if( 'all' == $dval ) {
		                                        $all_lockout += $v->lockout;
		                                    } else {
		                                        if ( isset( $delivery_days[ $dval ][$timeslot] ) ) {
		                                            $previous_lockout =  $delivery_days[ $dval ][$timeslot] + $v->lockout;
		                                            $delivery_days[ $dval ][$timeslot] = $previous_lockout;
		                                        } else {
		                                            $delivery_days[ $dval ][$timeslot] = $v->lockout;
		                                        }
		                                    }
		                                } else {
		                                    if ( isset( $specific_dates[ $dval ][$timeslot] ) ) {
		                                        $previous_lockout = $specific_dates[ $dval ][$timeslot] + $v->lockout;
		                                        $specific_dates[ $dval ][$timeslot] = $previous_lockout;
		                                    } else {
		                                        $specific_dates[ $dval ][$timeslot] = $v->lockout;
		                                    }
		                                }
		                            } else {
		                                $blank_lockout_time_slot = "yes";
		                            }
		                        }
		                    }
		                } else {
		                    if( $v->lockout != "" && $v->lockout != "0" ) {
		                        if( $v->tv == 'weekdays' ) {
		                        	if( 'all' == $v->dd ) {
			                            ...
			                            if ( isset( $delivery_days[ $v->dd ][$timeslot] ) ) {
			                                $previous_lockout =  $delivery_days[ $v->dd ][$timeslot] + $v->lockout;
			                                $delivery_days[ $v->dd ][$timeslot] = $previous_lockout;
			                            } else {
			                                $delivery_days[ $v->dd ][$timeslot] = $v->lockout;
			                            }
			                        }
		                        } else {
		                            if ( isset( $specific_dates[ $dval ][$timeslot] ) ) {
		                                $previous_lockout = $specific_dates[ $v->dd ][$timeslot] + $v->lockout;
		                                $specific_dates[ $v->dd ][$timeslot] = $previous_lockout;
		                            } else {
		                                $specific_dates[ $v->dd ][$timeslot] = $v->lockout;
		                            }
		                        }
		                    } else {
		                        $blank_lockout_time_slot = "yes";
		            ...
		            if ( is_array( $delivery_days ) &&  count( $delivery_days ) > 0 ) {;
		                foreach( $delivery_days as $del_k => $del_v ) {
							foreach( $del_v as $time => $lockout ) {
								$delivery_days[ $del_k ][$time] = $lockout + $all_lockout;
							}
		                }
		                $delivery_days[ 'all' ] = $all_lockout;
		            } else if( is_array( $delivery_days ) && count( $delivery_days ) == 0 ) {
		                $delivery_days[ 'all' ] = $all_lockout;
		            }
		            ...
		            foreach ( $lockout_timeslots_arr as $k => $v ) {
						...
						$timeslot = $v->t;
		                if ( isset( $date[$date_str][$timeslot] ) ) {
		                    $previous_orders =  $date[ $date_str ][$timeslot] + $v->o;
		                    $date[ $date_str ][$timeslot] = $previous_orders;
		                } else {
		                    $date[ $date_str ][$timeslot] = $v->o;
		                }
		            }
		            $lockout_arr = array();
		            foreach ( $date as $k => $v ) {
						foreach( $v as $timeslot => $lockout ) {
							if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && get_option( 'palpodelivery_enable_specific_delivery_dates' ) == 'on' ) {
								$specific_date = date( 'n-j-Y', strtotime( $k ) );
								if ( isset( $specific_dates[ $specific_date ][$timeslot] ) && $v[$timeslot] >= $specific_dates[ $specific_date ][$timeslot] && $specific_dates[ $specific_date ][$timeslot] != '' && $blank_lockout_time_slot != "yes" ) {
									if( isset( $lockout_arr[$k] ) ) {
										array_push( $lockout_arr[$k], $timeslot );
									}else {
										$lockout_arr[$k] = array( $timeslot );
									}
								}
							}
							...
							if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
								...
								if ( isset( $delivery_days[ 'palpodelivery_weekday_' . $weekday ][$timeslot] ) ) {
									if( $v[$timeslot] >= $delivery_days[ 'palpodelivery_weekday_' . $weekday ][$timeslot] && '' != $delivery_days[ 'palpodelivery_weekday_' . $weekday ][$timeslot]  && '0' != $delivery_days[ 'palpodelivery_weekday_' . $weekday ][$timeslot] && 'yes' != $blank_lockout_time_slot) {
										if( isset( $lockout_arr[$k] ) ) {
											array_push( $lockout_arr[$k], $timeslot );
										}else {
											$lockout_arr[$k] = array( $timeslot );
									...
									if( $v >= $delivery_days[ 'all' ] && '' != $delivery_days[ 'all' ]  && '0' != $delivery_days[ 'all' ] && 'yes' != $blank_lockout_time_slot ) {
										if( isset( $lockout_arr[$k] ) ) {
											array_push( $lockout_arr[$k], $timeslot );
										}else {
											$lockout_arr[$k] = array( $timeslot );
		            ...
		            foreach ( $lockout_arr as $k => $v ) {
						if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && get_option( 'palpodelivery_enable_specific_delivery_dates' ) == 'on' ) {
							$specific_date = date( 'n-j-Y', strtotime( $k ) );
							if( isset( $specific_dates[ $specific_date ] ) && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $specific_dates[ $specific_date ] ) ) {
								$lockout = explode( "-", $k );
								$lockout_days_str .= '"' . intval( $lockout[ 1 ] ) . '-' . intval( $lockout[ 0 ] ) . '-' . $lockout[ 2 ] . '",';
							}
						}

						if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
							$weekday = date( 'w', strtotime( $k ) );
							if ( isset( $delivery_days[ 'palpodelivery_weekday_' . $weekday ] )  && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $delivery_days[ 'palpodelivery_weekday_' . $weekday ] ) ) {
								$lockout = explode( "-", $k );
								$lockout_days_str .= '"' . intval( $lockout[ 1 ] ) . '-' . intval( $lockout[ 0 ] ) . '-' . $lockout[ 2 ] . '",';
							}
						}
					...
					if( 'on' == get_option('palpodelivery_enable_specific_delivery_dates') ) {
						$delivery_days = array();
						$delivery_dates = get_option( 'palpodelivery_delivery_dates' );
						if ( $delivery_dates != '' && $delivery_dates != '{}' && $delivery_dates != '[]' && $delivery_dates != 'null' ) {
							$delivery_days = json_decode( $delivery_dates );
						}

						$lockout_days = get_option( 'palpodelivery_lockout_days' );
						$lockout_days_arr = array();
						if ( $lockout_days != '' && $lockout_days != '{}' && $lockout_days != '[]' && $lockout_days != "null" ) {
							$lockout_days_arr = json_decode( get_option( 'palpodelivery_lockout_days' ) );
						}

						foreach( $delivery_days as $key => $value ) {
							$date = $value->date;
							$max_orders = isset( $value->max_orders ) ? $value->max_orders : '';
							array_push( $specific_dates, $value->date );
							if( is_array( $lockout_days_arr ) && count( $lockout_days_arr ) > 0 ) {
								foreach ( $lockout_days_arr as $k => $v ) {
									if ( $v->d == $date && $v->o >= $max_orders && $max_orders != '') {
										$lockout_date_split = explode( '-', $v->d );
										$lockout_date_timestamp = strtotime( $lockout_date_split[1] . "-" . $lockout_date_split[0] . "-" . $lockout_date_split[2] );
										if( $lockout_date_timestamp >= strtotime( $current_date ) ) {
					...
					if ( get_option( 'palpodelivery_lockout_date_after_orders' ) > 0 ) {
						$lockout_days_arr = array();
						$lockout_days = get_option( 'palpodelivery_lockout_days' );
						if ( $lockout_days != '' && $lockout_days != '{}' && $lockout_days != '[]' && $lockout_days != "null" ) {
							$lockout_days_arr = json_decode( get_option( 'palpodelivery_lockout_days' ) );
						...
						foreach ( $lockout_days_arr as $k => $v ) {
							$delivery_dates = json_decode(get_option( 'palpodelivery_delivery_dates' ), true);
							if( in_array( $v->d, $specific_dates ) ) {
								continue;
							}
							if ( $v->o >= get_option( 'palpodelivery_lockout_date_after_orders' ) ) {
		        ...
		        wp_cache_set( 'palpodelivery_general_lockout_days_str', $lockout_days_str );
		    }
	    }

	    $var .= '<input type="hidden" name="palpodelivery_lockout_days" id="palpodelivery_lockout_days" value=\'' . $lockout_days_str . '\'>';

	    //Min Date
	    $minimum_delivery_time = get_option( 'palpodelivery_minimumOrderDays' );
	    ...
	    $advance_booking_hrs =   get_option( 'palpodelivery_minimumOrderDays' );
	    if( 'on' == get_option( 'palpodelivery_enable_day_wise_settings' ) ) {
	    	$current_weekday = "palpodelivery_weekday_" . date( "w", $current_time );
	    	$advance_settings = get_option( 'palpodelivery_advance_settings' );
            if( '' == $advance_settings || '{}' == $advance_settings || '[]' == $advance_settings ) {
                $advance_settings = array();
            }
            foreach( $advance_settings as $ak => $av ) {
            	if( $current_weekday == $av[ 'palpodelivery_weekdays' ] ) {
            		if( '' != $av[ 'palpodelivery_minimumOrderDays' ] ) {
            			$delivery_time_seconds = $av[ 'palpodelivery_minimumOrderDays' ] * 60 * 60;
            			$advance_booking_hrs =   $av[ 'palpodelivery_minimumOrderDays' ];
            		}
            	}
            }
	    }

		$min_date_array = palpodelivery_common::get_min_date( $delivery_time_seconds, array( 'enabled' => get_option( 'palpodelivery_enable_delivery_time' ), 'from_hours' => get_option( 'palpodelivery_delivery_from_hours' ), 'to_hours' => get_option( 'palpodelivery_delivery_to_hours' ), 'from_mins' => get_option( 'palpodelivery_delivery_from_mins' ), 'to_mins' => get_option('palpodelivery_delivery_to_mins') ), $holidays_str, $lockout_days_str );
	    ...
	    if ( get_option( 'palpodelivery_enable_time_slot' ) == 'on' ) {
	        ...
	        foreach ( $existing_timeslots_arr as $k => $v ) {
	            ...
	            if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 ) {
					...
					$min_time_on_last_slot = apply_filters( 'palpodelivery_min_delivery_on_last_slot', false );
	                ...
	                if ( is_array( $dd ) && count( $dd ) > 0 ) {
	                    if( in_array( $check_min_date, $dd ) ) {
	                        ...
	                        $min_weekday = 'palpodelivery_weekday_' . $min_weekday;
	        ...
	        if( $last_slot_hrs != 0 ) {
		        ...
		        // if( 'on' != get_option ( 'palpodelivery_enable_same_day_delivery' ) && 'on' != get_option ( 'palpodelivery_enable_next_day_delivery' ) ) {
				...
				if ( 'on' == get_option( 'palpodelivery_enable_specific_delivery_dates' ) ) {
					$delivery_dates = get_option( 'palpodelivery_delivery_dates' );
					if ( $delivery_dates != '' && $delivery_dates != '{}' && $delivery_dates != '[]' && $delivery_dates != 'null' ) {
						$delivery_dates_arr = json_decode( get_option( 'palpodelivery_delivery_dates' ) );
					}
				}
	    		...
	    		if( "checked" != get_option( 'palpodelivery_weekday_' . $current_weekday ) && ( ( 'on' == get_option( 'palpodelivery_enable_specific_delivery_dates' ) && is_array( $dates_to_check ) && count( $dates_to_check ) > 0 && !in_array( $date_to_check, $dates_to_check ) ) || ( 'on' != get_option( 'palpodelivery_enable_specific_delivery_dates' ) ) ) ) {
		        ...
		        if ( version_compare( phpversion(), '5.3.0', '>' ) ) {
		            ...
		            $difference        =   palpodelivery_common::dateTimeDiff( $date2, $date1 );
		        }
		    	...
		    	if ( version_compare( phpversion(), '5.3.0', '>' ) ) {
		            ...
		            $calculated_difference        =   palpodelivery_common::dateTimeDiff( $calculated_min_date, $date1 );
		        }
	    ...
	    if ( get_option( 'palpodelivery_delivery_from_hours' ) != '' ) {
			if( $current_date == $min_date_array[ 'min_date' ] && 'on' == get_option( 'palpodelivery_enable_next_day_delivery' ) && 'on' != get_option('palpodelivery_enable_same_day_delivery' ) ) {
				$var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';
			}else if( ( get_option( 'palpodelivery_delivery_from_hours' ) < $current_hour || ( get_option( 'palpodelivery_delivery_from_hours' ) == $current_hour &&$current_minute > 0 ) ) && $current_date == $min_date_array[ 'min_date' ] && ( $current_hour > $min_date_array[ 'min_hour' ] || ( $current_hour == $min_date_array[ 'min_hour' ] && $current_minute > $min_date_array[ 'min_minute' ] ) ) ) {
				$var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . $current_hour . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . $current_minute . '">';
	        } else if( get_option( 'palpodelivery_delivery_from_hours' ) < $min_date_array[ 'min_hour' ] || ( get_option( 'palpodelivery_delivery_from_hours' ) == $min_date_array[ 'min_hour' ] && $min_date_array[ 'min_minute' ] > 0 ) ) {
	            $var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . $min_date_array[ 'min_hour' ] . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . $min_date_array[ 'min_minute' ] . '">';
	        } else if( get_option( 'palpodelivery_delivery_from_hours' ) > $current_hour ) {
	            $var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';
	        } else if( '0' == $min_date_array[ 'min_hour' ] && '0' == $min_date_array[ 'min_minute' ] && $current_date != $min_date_array['min_date' ] ) {
	        	$var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';
	        } else if( $min_date_array[ 'min_hour' ] == get_option( 'palpodelivery_delivery_from_hours' ) ) {
	        	$var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
	            $var .= '<input type="hidden" name="palpodelivery_min_minute" id="palpodelivery_min_minute" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';
	        } else {
	            $var .= '<input type="hidden" name="palpodelivery_min_hour" id="palpodelivery_min_hour" value="' . $current_hour . '">';
			...
			$var .= '<input type="hidden" name="palpodelivery_min_hour_set" id="palpodelivery_min_hour_set" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
			$var .= '<input type="hidden" name="palpodelivery_min_mins_set" id="palpodelivery_min_mins_set" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';
	    }

	    $var .= '<input type="hidden" name="palpodelivery_max_hour_set" id="palpodelivery_max_hour_set" value="' . get_option( 'palpodelivery_delivery_to_hours' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_max_mins_set" id="palpodelivery_max_mins_set" value="' . get_option( 'palpodelivery_delivery_to_mins' ) . '">';

		$var .= '<input type="hidden" name="palpodelivery_delivery_from_hours" id="palpodelivery_delivery_from_hours" value="' . get_option( 'palpodelivery_delivery_from_hours' ) . '">';
		$var .= '<input type="hidden" name="palpodelivery_delivery_from_mins" id="palpodelivery_delivery_from_mins" value="' . get_option( 'palpodelivery_delivery_from_mins' ) . '">';

	    $var .= '<input type="hidden" name="palpodelivery_current_day" id="palpodelivery_current_day" value="' . $current_date . '">';
	    $var .= '<input type="hidden" name="palpodelivery_current_hour" id="palpodelivery_current_hour" value="' . $current_hour . '">';
		$var .= '<input type="hidden" name="palpodelivery_current_minute" id="palpodelivery_current_minute" value="' . $current_minute . '">';

	    $var .= '<input type="hidden" name="palpodelivery_next_day" id="palpodelivery_next_day" value="' . $next_date . '">';


	    if( has_filter( 'palpodelivery_first_available_delivery_date' ) ) {
	    	$min_order_date = apply_filters( 'palpodelivery_first_available_delivery_date', $min_date_array[ 'min_date' ] );
	    } else {
	    	$min_order_date = $min_date_array[ 'min_date' ];
	    }

	    $var .= '<input type="hidden" name="palpodelivery_minimumOrderDays" id="palpodelivery_minimumOrderDays" value="' . $min_order_date . '">';
	    $var .= '<input type="hidden" name="palpodelivery_min_date_set" id="palpodelivery_min_date_set" >';
	    $var .= '<input type="hidden" name="palpodelivery_current_date_set" id="palpodelivery_current_date_set" value="' . $min_date_array[ 'current_date_to_check' ] . '">';
	    $var .= '<input type="hidden" name="palpodelivery_number_of_dates" id="palpodelivery_number_of_dates" value="' . get_option( 'palpodelivery_number_of_dates' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_number_of_months" id="palpodelivery_number_of_months" value="' . get_option('palpodelivery_number_of_months' ) . '">';

	    $var .= '<input type="hidden" name="palpodelivery_timeslot_field_mandatory" id="palpodelivery_timeslot_field_mandatory" value="' . get_option( 'palpodelivery_time_slot_mandatory' ) . '" >';
	    $var .= '<input type="hidden" name="palpodelivery_shipping_method_hidden_vars_arr" id="palpodelivery_shipping_method_hidden_vars_arr" value="">';
	    $var .= '<input type="hidden" id="palpodelivery_current_time" value="' . $current_time . '">';
	    $var .= '<input type="hidden" id="palpodelivery_same_day_delivery" name = "palpodelivery_same_day_delivery" value="' . __( get_option( 'palpodelivery_enable_same_day_delivery' ), 'order-delivery-date' ) . '">';
	    $var .= '<input type="hidden" id="palpodelivery_next_day_delivery" name = "palpodelivery_next_day_delivery" value="' . __( get_option( 'palpodelivery_enable_next_day_delivery' ), 'order-delivery-date' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_time_settings_selected" id="palpodelivery_time_settings_selected">';
	    ...
	    $var .= '<input type="hidden" name="palpodelivery_admin_url" id="palpodelivery_admin_url" value="' . $ajax_url . '">';
	    $var .= '<input type="hidden" name="palpodelivery_is_admin" id="palpodelivery_is_admin" value="' . is_admin() . '">';
	    $var .= '<input type="hidden" name="palpodelivery_is_cart" id="palpodelivery_is_cart" value="' . is_cart() . '">';
	    $var .= '<input type="hidden" name="palpodelivery_is_account_page" id="palpodelivery_is_account_page" value="' . is_account_page() . '">';
	    $var .= '<input type="hidden" name="palpodelivery_delivery_date_on_cart_page" id="palpodelivery_delivery_date_on_cart_page" value="' . get_option( 'palpodelivery_delivery_date_on_cart_page' ) . '">';

	    $palpodelivery_date_field_label = get_option( "palpodelivery_delivery_date_field_label" );
	    if( '' == $palpodelivery_date_field_label ) {
	    	$palpodelivery_date_field_label = palpodelivery_DELIVERY_DATE_FIELD_LABEL;
	    }
	    $var .= '<input type="hidden" name="palpodelivery_field_label" id="palpodelivery_field_label" value="' . $palpodelivery_date_field_label . '">';

	    $palpodelivery_timeslot_field_label = get_option( "palpodelivery_delivery_timeslot_field_label" );
	    if( '' == $palpodelivery_timeslot_field_label ) {
	    	$palpodelivery_timeslot_field_label = palpodelivery_DELIVERY_TIMESLOT_FIELD_LABEL;
	    }
	    $var .= '<input type="hidden" name="palpodelivery_timeslot_field_label" id="palpodelivery_timeslot_field_label" value="' . $palpodelivery_timeslot_field_label . '">';
	    $var .= '<input type="hidden" name="palpodelivery_field_name" id="palpodelivery_field_name" value="' . $field_name . '">';

	    $var .= '<input type="hidden" name="palpodelivery_auto_populate_first_available_time_slot" id="palpodelivery_auto_populate_first_available_time_slot" value="' . get_option( 'palpodelivery_auto_populate_first_available_time_slot' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_delivery_time_format" id="palpodelivery_delivery_time_format" value="' . get_option( 'palpodelivery_delivery_time_format' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_enable_autofill_of_delivery_date" id="palpodelivery_enable_autofill_of_delivery_date" value="' . get_option( 'palpodelivery_enable_autofill_of_delivery_date' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_enable_shipping_based_delivery" id="palpodelivery_enable_shipping_based_delivery" value="' . get_option( 'palpodelivery_enable_shipping_based_delivery' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_enable_shipping_days" id="palpodelivery_enable_shipping_days" value="' . get_option( 'palpodelivery_enable_shipping_days' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_date_field_mandatory" id="palpodelivery_date_field_mandatory" value="' . get_option( 'palpodelivery_date_field_mandatory' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_enable_availability_display" id="palpodelivery_enable_availability_display" value="' . get_option( 'palpodelivery_enable_availability_display' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_show_partially_booked_dates" id="palpodelivery_show_partially_booked_dates" value="' . get_option( 'palpodelivery_show_partially_booked_dates' ) . '">';

	    $allshippingdays = array();
	    foreach ( $palpodelivery_shipping_days as $n => $day_name ) {
	    ...
	    if ( $shipping_checked == 'Yes' ) {
	   	...
	   	$zone_id = '';
		$palpodelivery_shipping_id = '';
		$order_id = '';
		if( is_admin() && isset( $post->ID ) && '' != $post->ID && ( 'shop_order' == $post->post_type  || 'shop_subscription' == $post->post_type ) ) {
			$order_id = $post->ID;
			$zone_details = explode( "-", palpodelivery_common::palpodelivery_get_zone_id( $order_id, false ) );
			$zone_id = $zone_details[ 0 ];
			$palpodelivery_shipping_id = $zone_details[ 1 ];

		}

        $var .= "<input type='hidden' name='palpodelivery_zone_id' id='palpodelivery_zone_id' value='" . $zone_id . "'>";
        $var .= "<input type='hidden' name='palpodelivery_shipping_id' id='palpodelivery_shipping_id' value='" . $palpodelivery_shipping_id . "'>";

	    $categories = palpodelivery_common::palpodelivery_get_cart_product_categories( $order_id );
	    $category_to_send = implode( ",", $categories );

	    $var .= '<input type="hidden" name="palpodelivery_category_settings_to_load" id="palpodelivery_category_settings_to_load" value="' . $category_to_send . '">';

	    $shipping_classes = palpodelivery_common::palpodelivery_get_cart_shipping_classes( $order_id );
		$shipping_class_to_send = implode( ",", $shipping_classes );

	    $var .= '<input type="hidden" name="palpodelivery_shipping_class_settings_to_load" id="palpodelivery_shipping_class_settings_to_load" value="' . $shipping_class_to_send . '">';
	    ...
	    $get_common_delivery_days = palpodelivery_common::palpodelivery_common_delivery_days_for_product_category( $palpodelivery_shipping_id );

	    $get_common_delivery_days_json = '';
	    $delivery_dates_str = '';
	    $palpodelivery_is_days_common = '';
	    $palpodelivery_categories_settings_common = '';
	    $holidays_str = '';
	    $locked_days_str = '';

        if( is_array( $get_common_delivery_days[ 'palpodelivery_common_weekdays' ] ) && count( $get_common_delivery_days[ 'palpodelivery_common_weekdays' ] ) > 0 ) {
            $get_common_delivery_days_json = json_encode( $get_common_delivery_days[ 'palpodelivery_common_weekdays' ] );
        }

		$var .= '<input type=\'hidden\' name=\'palpodelivery_common_delivery_days_for_product_category\' id=\'palpodelivery_common_delivery_days_for_product_category\' value=\'' . $get_common_delivery_days_json . '\'>';

        if( is_array( $get_common_delivery_days[ 'palpodelivery_common_specific_dates' ] ) && count( $get_common_delivery_days[ 'palpodelivery_common_specific_dates' ] ) > 0 ) {
        	foreach( $get_common_delivery_days[ 'palpodelivery_common_specific_dates' ] as $key => $value ) {
                $delivery_dates_str .= '"' . $value . '",';
            }
            $delivery_dates_str = substr( $delivery_dates_str, 0, strlen( $delivery_dates_str )-1 );
        }

		$var .= '<input type=\'hidden\' name=\'palpodelivery_common_delivery_dates_for_product_category\' id=\'palpodelivery_common_delivery_dates_for_product_category\' value=\'' . $delivery_dates_str . '\'>';

        if( is_array( $get_common_delivery_days[ 'palpodelivery_common_holidays' ] ) && count( $get_common_delivery_days[ 'palpodelivery_common_holidays' ] ) > 0 ) {
        	foreach( $get_common_delivery_days[ 'palpodelivery_common_holidays' ] as $key => $value ) {
                $holidays_str .= '"' . $value . '",';
            }
            $holidays_str = substr( $holidays_str, 0, strlen( $holidays_str )-1 );
        }

		$var .= '<input type=\'hidden\' name=\'palpodelivery_common_holidays_for_product_category\' id=\'palpodelivery_common_holidays_for_product_category\' value=\'' . $holidays_str . '\'>';


        if( is_array( $get_common_delivery_days[ 'palpodelivery_common_locked_days' ] ) &&  count( $get_common_delivery_days[ 'palpodelivery_common_locked_days' ] ) > 0 ) {
        	foreach( $get_common_delivery_days[ 'palpodelivery_common_locked_days' ] as $key => $value ) {
                $locked_days_str .= '"' . $value . '",';
            }
            $locked_days_str = substr( $locked_days_str, 0, strlen( $holidays_str )-1 );
        }

		$var .= '<input type=\'hidden\' name=\'palpodelivery_common_locked_days\' id=\'palpodelivery_common_locked_days\' value=\'' . $locked_days_str . '\'>';

		if( isset( $get_common_delivery_days[ 'palpodelivery_is_days_common' ] ) ) {
			$palpodelivery_is_days_common = $get_common_delivery_days[ 'palpodelivery_is_days_common' ];
		}

		$var .= '<input type=\'hidden\' name=\'palpodelivery_is_days_common\' id=\'palpodelivery_is_days_common\' value=\'' . $palpodelivery_is_days_common . '\'>';

		if( isset( $get_common_delivery_days[ 'palpodelivery_categories_settings_common' ] ) ) {
			$palpodelivery_categories_settings_common = $get_common_delivery_days[ 'palpodelivery_categories_settings_common' ];
		}

		$var .= '<input type=\'hidden\' name=\'palpodelivery_categories_settings_common\' id=\'palpodelivery_categories_settings_common\' value=\'' . $palpodelivery_categories_settings_common . '\'>';

	    // Compatibility with WooCommerce Subscriptions plugin.
	    if ( class_exists( 'WC_Subscriptions' ) && get_option( 'palpodelivery_enable_woo_subscriptions_compatibility' ) == 'on' && get_option( 'palpodelivery_woocommerce_subscriptions_compatibility' ) == 'on' ) {
	    	if ( class_exists( 'ws_addon_for_palpodelivery' ) ) {
        		$subscrition_var = ws_addon_for_palpodelivery::palpodelivery_check_the_subscription_period();
        	}
	        if( isset( $subscrition_var[ 'palpodelivery_if_renewal_subscription' ] ) ) {
	            $var .= '<input type="hidden" name="palpodelivery_if_renewal_subscription" id="palpodelivery_if_renewal_subscription" value="yes">';
	        }
	        if( isset( $subscrition_var[ 'palpodelivery_number_of_dates_for_subscription' ] ) ) {
	            $var .= '<input type="hidden" name="palpodelivery_number_of_dates_for_subscription" id="palpodelivery_number_of_dates_for_subscription" value="' . $subscrition_var[ 'palpodelivery_number_of_dates_for_subscription' ] . '">';
	        }
	        if( isset( $subscrition_var[ 'palpodelivery_start_date_for_subscription' ] ) ) {
                $delivery_time_seconds = $minimum_delivery_time * 60 * 60;
                $current_hour = date( 'H:i', $current_time );
                $min_timestamp = strtotime( $subscrition_var[ 'palpodelivery_start_date_for_subscription' ] . " " . $current_hour );
                if( get_option( 'palpodelivery_minimumOrderDays' ) != 0 && get_option( 'palpodelivery_minimumOrderDays' ) != '' ) {
                   $min_date_array = palpodelivery_common::get_min_date( $delivery_time_seconds, array( 'enabled' => get_option( 'palpodelivery_enable_delivery_time' ), 'from_hours' => get_option( 'palpodelivery_delivery_from_hours' ), 'to_hours' => get_option( 'palpodelivery_delivery_to_hours' ) ), $holidays_str, $locked_days_str );
                }
                $var .=  '<input type="hidden" name="palpodelivery_start_date_for_subscription" 	id="palpodelivery_start_date_for_subscription" value="' . $min_date_array[ 'min_date' ] . '">';
	        }
	        $var .= '<input type="hidden" name="palpodelivery_subscriptions_settings" id="palpodelivery_subscriptions_settings" value="' . get_option( 'palpodelivery_enable_woo_subscriptions_compatibility' ) . '">';
	    }

	    $alldays = array();
        foreach ( $palpodelivery_weekdays as $n => $day_name ) {
	    ...
	    } else if ( $checked == 'No' &&  get_option( 'palpodelivery_enable_specific_delivery_dates' ) != 'on') {
	       foreach ( $alldayskeys as $key ) {
	    ...
	    if( 'No' == $checked ) {
	    	$var .= '<input type="hidden" id="palpodelivery_is_all_weekdays_disabled" name="palpodelivery_is_all_weekdays_disabled" value="yes">';
	    } else {
	    	$var .= '<input type="hidden" id="palpodelivery_is_all_weekdays_disabled" name="palpodelivery_is_all_weekdays_disabled" value="no">';
	    }

	    $var .= "<input type='hidden' name='palpodelivery_load_delivery_date_var' id='palpodelivery_load_delivery_date_var' value='" . json_encode( $load_delivery_date_var ) . "'>";

	    $var .= '<input type="hidden" name="palpodelivery_specific_delivery_dates" id="palpodelivery_specific_delivery_dates" value="' . get_option( 'palpodelivery_enable_specific_delivery_dates' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_recurring_days" id="palpodelivery_recurring_days" value="' . get_option( 'palpodelivery_enable_specific_delivery_dates' ) . '">';

	    $disable_for_delivery_days = 'no';
        if( has_filter( 'palpodelivery_to_calculate_minimum_hours_for_disable_days' ) ) {
	       $disable_for_delivery_days = apply_filters( 'palpodelivery_to_calculate_minimum_hours_for_disable_days', $disable_for_delivery_days );
        }

	    $var .= '<input type="hidden" name="palpodelivery_disable_for_delivery_days" id="palpodelivery_disable_for_delivery_days" value="' . $disable_for_delivery_days . '">';

	    $palpodelivery_disable_for_holidays = 'no';
	    if( has_filter( 'palpodelivery_to_calculate_minimum_hours_for_holidays' ) ) {
	        $palpodelivery_disable_for_holidays = apply_filters( 'palpodelivery_to_calculate_minimum_hours_for_holidays', $palpodelivery_disable_for_holidays );
	    }

	    $var .= '<input type="hidden" name="palpodelivery_disable_for_holidays" id="palpodelivery_disable_for_holidays" value="' . $palpodelivery_disable_for_holidays . '">';

	    //fetch specific delivery dates
	    $delivery_dates_str = "";
        if ( get_option( 'palpodelivery_enable_specific_delivery_dates' ) == "on" ) {
            $delivery_dates_arr = array();
            $delivery_dates 	= get_option( 'palpodelivery_delivery_dates' );
            if ( $delivery_dates != '' && $delivery_dates != '{}' && $delivery_dates != '[]' && $delivery_dates != 'null' ) {
            ...
            usort( $delivery_dates_arr, array( 'palpodelivery_common', 'bkap_sort_specific_dates_array' ) );
	    ...
	    $var .= '<input type="hidden" name="palpodelivery_delivery_dates" id="palpodelivery_delivery_dates" value=\'' . $delivery_dates_str . '\'>';

	    $var .= '<input type="hidden" name="palpodelivery_enable_time_slot" id="palpodelivery_enable_time_slot" value="' . get_option( 'palpodelivery_enable_time_slot' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_enable_time_slider" id="palpodelivery_enable_time_slider" value="' . get_option( 'palpodelivery_enable_delivery_time' ) . '">';
	    $var .= '<input type="hidden" name="palpodelivery_time_slider_min_time" id="palpodelivery_time_slider_min_time">';
	    $disable_minimum_delivery_time_slider = '';

	    if( has_filter( 'palpodelivery_disable_minimum_delivery_time_slider' ) ) {
	        $disable_minimum_delivery_time_slider = apply_filters( 'palpodelivery_disable_minimum_delivery_time_slider', '' );
	    }

	    $var .= '<input type="hidden" name="palpodelivery_disable_minimum_delivery_time_slider" id="palpodelivery_disable_minimum_delivery_time_slider" value="' . $disable_minimum_delivery_time_slider . '">';

	    $shipping_methods_for_custom = palpodelivery_common::palpodelivery_shipping_methods_for_custom();
	    $var .= '<input type="hidden" name="palpodelivery_shipping_method_based_settings" id="palpodelivery_shipping_method_based_settings" value="' . $shipping_methods_for_custom . '">';

	    $disabled_days = palpodelivery_common::get_disabled_days();
	    $disabled_days_str = '';
	    ...
	    $var .= '<input type="hidden" name="palpodelivery_disabled_days_str" id="palpodelivery_disabled_days_str" value="' . $disabled_days_str . '">';
	    $var .= '<input type="hidden" name="palpodelivery_disabled_weekdays_str" id="palpodelivery_disabled_weekdays_str" value="' . $disabled_weekdays_str . '">';

	    $options = palpodelivery_common::get_datepicker_options( $holidays_str, $lockout_days_str );
	    $var .= '<input type="hidden" name="palpodelivery_option_str" id="palpodelivery_option_str" value="' . $options . '">';

	    $var .= '<input type="hidden" name="palpodelivery_delivery_date_format" id="palpodelivery_delivery_date_format" value="' . get_option( 'palpodelivery_delivery_date_format' ) . '">';

	    $show = 'datepicker';
	    if ( get_option( 'palpodelivery_enable_delivery_time' ) == 'on' ) {
	        if ( get_option( 'palpodelivery_enable_delivery_date' ) == 'on' ) {
	            $show = 'datetimepicker';
	        } else {
	            $show = 'timepicker';
	        }
	    }
		$var .= '<input type="hidden" name="palpodelivery_show_datepicker" id="palpodelivery_show_datepicker" value="' . $show . '">';

		$first_day_of_week = '';
		if( get_option( 'start_of_week' ) != '' ) {
			$first_day_of_week = get_option( 'start_of_week' );
		}
		$var .= '<input type="hidden" name="palpodelivery_start_of_week" id="palpodelivery_start_of_week" value="' . $first_day_of_week . '">';

		$field_note_text = __( get_option( 'palpodelivery_delivery_date_field_note' ), 'order-delivery-date' );
        $field_note_text = str_replace( array( "\r\n", "\r", "\n" ), "<br/>", $field_note_text );
        if( strpos( $field_note_text, '"' ) !== false ) {
            $var .= '<input type="hidden" name="palpodelivery_field_note_text" id="palpodelivery_field_note_text" value=\'' . $field_note_text . '\'>';
        } else {
            $var .= '<input type="hidden" name="palpodelivery_field_note_text" id="palpodelivery_field_note_text" value="' . $field_note_text . '">';
        }

		$hidden_vars_str = esc_attr( palpodelivery_common::palpodelivery_get_shipping_based_settings() );
		$var .= '<input type="hidden" name="palpodelivery_hidden_vars_str" id="palpodelivery_hidden_vars_str" value="' . $hidden_vars_str . '">';

        $var .= "<input type='hidden' name='palpodelivery_delivery_checkout_options' id='palpodelivery_delivery_checkout_options' value='" . get_option( 'palpodelivery_delivery_checkout_options' ) . "' >";

        $var .= "<input type='hidden' name='palpodelivery_minimum_delivery_time' id='palpodelivery_minimum_delivery_time' value='" . get_option( 'palpodelivery_minimumOrderDays' ) . "' >";

        if ( 'text_block' === get_option( 'palpodelivery_delivery_checkout_options' ) ) {
	        $var .= "<input type='hidden' name='palpodelivery_min_between_days' id='palpodelivery_min_between_days' value='" . get_option( 'palpodelivery_min_between_days' ) . "' >";
	        $var .= "<input type='hidden' name='palpodelivery_max_between_days' id='palpodelivery_max_between_days' value='" . get_option( 'palpodelivery_max_between_days' ) . "' >";
	        $var .= "<input type='hidden' name='palpodelivery_is_shipping_text_block' id='palpodelivery_is_shipping_text_block' value='' >";
    	}

        $disable_delivery_fields = 'no';
        if( has_filter( 'palpodelivery_disable_delivery_fields' ) ) {
	       $disable_delivery_fields = apply_filters( 'palpodelivery_disable_delivery_fields', $disable_delivery_fields );
        }

	    $var .= '<input type="hidden" name="palpodelivery_disable_delivery_fields" id="palpodelivery_disable_delivery_fields" value="' . $disable_delivery_fields . '">';

     	$partially_booked_dates_str = palpodelivery_widget::get_partially_booked_dates( '' );
	    $available_deliveries_arr = explode( '&', $partially_booked_dates_str );

	    $var .= '<input type="hidden" id="palpodelivery_partially_booked_dates" class="palpodelivery_partially_booked_dates" value="' . htmlspecialchars( $available_deliveries_arr[0] ) . '"/>';

	    if( isset( $available_deliveries_arr[1] ) ) {
	    	$var .= '<input type="hidden" id="palpodelivery_available_deliveries" name="palpodelivery_available_deliveries" value="' . htmlspecialchars( $available_deliveries_arr[1] ) . '">';
	    }

	    $add_tooltip_for_weekday_str = '';
        if( has_filter( 'palpodelivery_tooltip_for_weekday' ) ) {
			$add_tooltip_for_weekday = apply_filters( 'palpodelivery_tooltip_for_weekday', '' );
		 ...
		 *
		 * add_filter( 'palpodelivery_first_autofil_delivery_date', 'palpodelivery_first_autofil_delivery_date_callback' );
		 * function palpodelivery_first_autofil_delivery_date_callback() {
		 * 		return '29-6-2019';
		 * }
	     */
	    $first_autofil_date = '';
	    if( has_filter( 'palpodelivery_first_autofil_delivery_date' ) ) {
	    	$first_autofil_date = apply_filters( 'palpodelivery_first_autofil_delivery_date', '' );
	    }

	    $var .= '<input type="hidden" name="palpodelivery_first_autofil_delivery_date" id="palpodelivery_first_autofil_delivery_date" value="' . $first_autofil_date . '">';

		return apply_filters( 'palpodelivery_hidden_variables', $var );
	}
	...
	public static function palpodelivery_get_zone_id( $order_id = "", $is_ajax = true ) {
		$post_array = array();
		if( $order_id != "" ) {
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_shipping_methods_for_custom() {
	    global $wpdb;
	    $shipping_method_based_settings = 'no';
	    if ( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' &&
	    	get_option( 'palpodelivery_enable_delivery_date' ) == 'on' ) {
	...
	public static function get_datepicker_options( $holidays_str, $lockout_str ) {
	    ...
	    if ( get_option( 'palpodelivery_enable_delivery_date' ) == 'on' && ( get_option( 'palpodelivery_enable_same_day_delivery' ) == 'on' || get_option( 'palpodelivery_enable_next_day_delivery' ) == 'on' ) ) {
	        $options[] = "beforeShow:maxdt";
	    } else if ( get_option( 'palpodelivery_enable_next_day_delivery' ) != 'on' && get_option( 'palpodelivery_enable_same_day_delivery' ) != 'on' && get_option( 'palpodelivery_enable_delivery_date' ) == 'on' ) {
	        $options[] = "beforeShow:avd";
	    }

	    if ( get_option( 'palpodelivery_enable_delivery_date' ) == 'on' ) {
	        $options[] = "dateFormat:'" . get_option( 'palpodelivery_delivery_date_format' ) . "'";
	    }

	    $time_settings_enable = get_option( 'palpodelivery_enable_delivery_time' );
	    if ( $time_settings_enable == 'on' ) {
	        ...
	        $delivery_time_seconds = $minimum_delivery_time * 60 * 60;
			$min_date_array = palpodelivery_common::get_min_date( $delivery_time_seconds, array( 'enabled' => $time_settings_enable, 'from_hours' => get_option( 'palpodelivery_delivery_from_hours' ),'from_mins' => get_option( 'palpodelivery_delivery_from_mins' ), 'to_hours' => get_option( 'palpodelivery_delivery_to_hours' ), 'to_mins' => get_option( 'palpodelivery_delivery_to_mins' ) ), $holidays_str, $lockout_str );

	        if( !is_account_page() && !is_admin() ) {
		        if ( get_option( 'palpodelivery_delivery_from_hours' ) != '' ) {
					if( $current_date == $min_date_array[ 'min_date' ] && 'on' == get_option('palpodelivery_enable_next_day_delivery' ) && 'on' != get_option('palpodelivery_enable_same_day_delivery' ) ) {
						$options[] = "hourMin:" . get_option( 'palpodelivery_delivery_from_hours' );
		                $options[] = "minuteMin:" . get_option( 'palpodelivery_delivery_from_mins' );
					}elseif( ( get_option( 'palpodelivery_delivery_from_hours' ) < $current_hour || ( get_option( 'palpodelivery_delivery_from_hours' ) == $current_hour && $current_minute > 0 ) ) && $current_date == $min_date_array[ 'min_date' ] && ( $current_hour > $min_date_array[ 'min_hour' ] || ( $current_hour == $min_date_array[ 'min_hour' ] && $current_minute > $min_date_array[ 'min_minute' ] ) ) && $current_hour <= get_option( 'palpodelivery_delivery_to_hours' ) ) {
			            $options[] = "hourMin:" . $current_hour;
			            $options[] = "minuteMin:" . $current_minute;
			        } else if( get_option( 'palpodelivery_delivery_from_hours' ) < $min_date_array[ 'min_hour' ] || ( get_option( 'palpodelivery_delivery_from_hours' ) == $min_date_array[ 'min_hour' ] && $min_date_array[ 'min_minute' ] > 0 ) && $min_date_array[ 'min_hour' ] <= get_option( 'palpodelivery_delivery_to_hours' ) ) {
	        ...
	        if ( get_option( 'palpodelivery_delivery_to_hours' ) != '' ) {
	            $options[] = "hourMax:". get_option( 'palpodelivery_delivery_to_hours' );
	            $options[] = "minuteMax:". get_option( 'palpodelivery_delivery_to_mins' );
	        }

	        $step_min = apply_filters( 'palpodelivery_time_slider_minute_step', 5 );
	        $options[] = "stepMinute:" . $step_min;

	        if ( get_option( 'palpodelivery_delivery_time_format' ) == '1' ) {
	...
	public static function get_disabled_days() {
		global $palpodelivery_shipping_days, $palpodelivery_weekdays;
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
		...
		$minimum_delivery_time = palpodelivery_get_minimum_delivery_time();

	    if ( get_option( 'palpodelivery_enable_same_day_delivery' ) == 'on' ) {
			$same_day_cut_off = palpodelivery_get_cutoff_timestamp();
			$cut_off_with_min_time = $current_time + $minimum_delivery_time;

			$enable_day_until_cutoff = apply_filters( 'palpodelivery_enable_day_until_cutoff', false );
			if( $enable_day_until_cutoff && ( $minimum_delivery_time != 0 || $minimum_delivery_time != '' ) ) {
				$cut_off_with_min_time = $current_time;
			}

	        if( get_option( 'palpodelivery_enable_shipping_days' ) == 'on' ) {
	            $days_disabled = "No";
	            $palpodelivery_weekdays_enabled = array();
	            foreach ( $palpodelivery_shipping_days as $s_key => $s_value ) {
	                $day_check = get_option( $s_key );
	                if( $day_check == "checked" ) {
	                    $palpodelivery_weekdays_enabled[ $s_key ] = $day_check;
	                }
	            }
	            ...
	            if ( $cut_off_with_min_time < $same_day_cut_off ) {
	                ...
	                if( !in_array( date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time ), $holiday_date_arr ) ) {
	                	$disabled_days[] = date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time );
	                }
	            }
	        } else {
	            if ( $cut_off_with_min_time < $same_day_cut_off ) {
		...
		if ( get_option( 'palpodelivery_enable_next_day_delivery' ) == 'on' ) {
			$next_day_cut_off = palpodelivery_get_cutoff_timestamp('next_day');

			$cut_off_with_min_time = $current_time + $minimum_delivery_time;

	        if( 'on' == get_option( 'palpodelivery_enable_day_wise_settings' ) ) {
                $current_weekday = "palpodelivery_weekday_" . date( "w", $current_time );
				$advance_settings = false !== get_option( 'palpodelivery_advance_settings') ? get_option( 'palpodelivery_advance_settings') : array();

				if( '' !== $advance_settings && '{}' !== $advance_settings && '[]' !== $advance_settings  ) {
					foreach( $advance_settings as $ak => $av ) {
						if( $current_weekday == $av[ 'palpodelivery_weekdays' ] ) {
							if( "" != $av[ 'palpodelivery_disable_next_day_delivery_after_hours' ] ) {
								$cut_off_time = explode( ":", $av[ 'palpodelivery_disable_next_day_delivery_after_hours' ] );
								$cut_off_hour = $cut_off_time[0];
								$cut_off_minute = $cut_off_time[1];
							}

							$var .= '<input type="hidden" name="palpodelivery_before_cutoff_weekday" id="palpodelivery_before_cutoff_weekday" value="' . $av[ 'palpodelivery_before_cutoff_weekday' ] . '">';
							$var .= '<input type="hidden" name="palpodelivery_after_cutoff_weekday" id="palpodelivery_after_cutoff_weekday" value="' . $av[ 'palpodelivery_after_cutoff_weekday' ] . '">';
						}
					}
				}
          	...
          	if( get_option( 'palpodelivery_enable_shipping_days' ) == 'on' ) {
	        	...
	        	$days_disabled = "No";
	            $palpodelivery_weekdays_enabled = array();
	            foreach ( $palpodelivery_weekdays as $s_key => $s_value ) {
	                $day_check = get_option( $s_key );
	                if( $day_check == "checked" ) {
	                    $palpodelivery_weekdays_enabled[ $s_key ] = $day_check;
	                }
			...
			if ( $current_time < $next_day_cut_off && $cut_off_with_min_time < strtotime( $day_after ) ) {
                ...
                for ( $j = $current_weekday; $j <= 6; ) {
                    $is_day_disabled = 'no';
                	if( get_option( 'palpodelivery_enable_shipping_days' ) == 'on' ) {
                		if( is_array( $palpodelivery_weekdays_enabled ) && count( $palpodelivery_weekdays_enabled ) > 0 && !isset( $palpodelivery_weekdays_enabled[ 'palpodelivery_shipping_day_' . $j ] ) ) {
                			$is_day_disabled = 'yes';
                    	}
                	} else {
                		if( is_array( $palpodelivery_weekdays_enabled ) && count( $palpodelivery_weekdays_enabled ) > 0 && !isset( $palpodelivery_weekdays_enabled[ 'palpodelivery_weekday_' . $j ] ) ) {
							$is_day_disabled = 'yes';
                    	}
                	}
                ...
                if ( get_option( 'palpodelivery_enable_same_day_delivery' ) != 'on' ) {
                	$disable_weekdays[] = date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time );
                }
            	...
            	for ( $j = $next_day; $j <= 6; ) {
                	...
                	if( $is_day_disabled == 'yes' ) {
                    	...
                    	if( $current_time <= $next_day_timestamp ) {
	                    	if( in_array( date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_day_timestamp ), $holiday_date_arr ) ) {
								$days_disabled = "Yes";
								$disable_weekdays[] = date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time );
                ...
                if( $days_disabled == "Yes" ) {
                    $disabled_days[] = date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_day_timestamp );
                } else {
                    $disabled_days[] = date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_day_timestamp );
                }
            }
	 ...
	 * @since 3.2
	 */
	public static function palpodelivery_cancel_delivery_for_trashed( $order_id ) {
	    global $typenow;
	    if ( 'shop_order' != $typenow && ( !isset( $_POST[ 'palpodelivery_post_type' ] ) || ( isset( $_POST[ 'palpodelivery_post_type' ] ) && ( $_POST[ 'palpodelivery_post_type' ] != 'shop_order' ) ) ) ) {
            return;
	    } else {
			...
			if ( 'shop_order' == $post_obj->post_type && ( !in_array( $post_obj->post_status, $status ) ) ) {
	        	palpodelivery_common::palpodelivery_cancel_delivery( $order_id );
	    	}
	    }
	}
	 ...
	 * @since 7.5
	 */
	public static function palpodelivery_untrash_order( $post_id ) {
	    $post_obj = get_post( $post_id );
	    $status = array( 'wc-cancelled', 'wc-refunded', 'wc-failed' );
	    if ( 'shop_order' == $post_obj->post_type && ( !in_array( $post_obj->post_status, $status ) ) ) {
	        // untrash the booking posts as well
	        palpodelivery_common::palpodelivery_restore_deliveries( $post_id, 'trashed', $post_obj->post_status );
	    }
	}
	...
	* @since 7.5
	*/
	public static function palpodelivery_restore_deliveries( $order_id, $old_status, $new_status ) {
		if( ( $old_status == 'cancelled' && ( $new_status != 'cancelled' || $new_status != 'refunded' || $new_status != 'failed' ) )
	        ...
	        $time_field_label = palpodelivery_common::palpodelivery_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location, $order_id );

            if( isset( $data[ '_palpodelivery_timestamp' ][ 0 ] ) && $data[ '_palpodelivery_timestamp' ][ 0 ] != '' ) {
                $delivery_date = date( "j-n-Y", $data[ '_palpodelivery_timestamp' ][ 0 ] );
                palpodelivery_process::palpodelivery_update_lockout_days( $delivery_date );
                if( isset( $data[ $time_field_label ][ 0 ] ) && $data[ $time_field_label ][ 0 ] != '' ) {
                	$time_slot = $data[ $time_field_label ][ 0 ];
                	palpodelivery_process::palpodelivery_update_time_slot( $time_slot, $delivery_date );
            	}
            }
		}
	}
	 ...
	 * @since 3.2
	 */
    public static function palpodelivery_cancel_delivery( $order_id ) {
	    global $wpdb, $typenow;
	    $shipping_method = '';

	    $delivery_date_timestamp = '';
	    $post_meta = get_post_meta( $order_id, '_palpodelivery_timestamp' );
        ...
        $time_field_label = palpodelivery_common::palpodelivery_get_delivery_time_field_label( $shipping_method, $categories, $shipping_class, $location, $order_id );
        $timeslt = '';
	    ...
	    $total_quantities = 1;
	    if( get_option( 'palpodelivery_lockout_date_quantity_based' ) == 'on' ) {
            $order = new WC_Order( $order_id );
            $items = $order->get_items();
            $total_quantities = 0;
            foreach( $items as $key => $value ) {
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
	    	foreach ( $results as $key => $value ) {
                $shipping_methods = array();
                $shipping_settings = get_option( $value->option_name );
                if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'palpodelivery_locations' ) {

                    if( isset( $shipping_settings[ 'palpodelivery_locations' ] ) && in_array( $location, $shipping_settings[ 'palpodelivery_locations' ] ) ) {
			...
			if( 'No' == $shipping_based_lockout ) {
				foreach ( $results as $key => $value ) {
					...
					$shipping_settings = get_option( $value->option_name );
					if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
						if( has_filter( 'palpodelivery_get_shipping_method' ) ) {
							$shipping_methods_values = apply_filters( 'palpodelivery_get_shipping_method', $shipping_settings, $_POST, $shipping_settings[ 'shipping_methods' ], $shipping_method );
            ...
            if( is_array( $shipping_settings_to_check ) && count( $shipping_settings_to_check ) > 0 ) {
            	foreach ( $shipping_settings_to_check as $setting_key => $setting_value ) {
	            	if ( $setting_value[ 'date_lockout' ] != '' && $setting_value[ 'date_lockout' ] != '0' ) {
	                    if( $delivery_date_timestamp != '' ) {
	                        $delivery_date = date( palpodelivery_LOCKOUT_DATE_FORMAT, $delivery_date_timestamp );
	                        if( isset( $setting_value[ 'palpodelivery_lockout_date' ] ) ) {
	                            $lockout_date_array = $setting_value[ 'palpodelivery_lockout_date' ];
	                        ...
	                        $setting_value[ 'palpodelivery_lockout_date' ] = json_encode( $lockout_date_arr );
	                ...
	                if( isset( $setting_value[ 'time_slots' ] ) ) {
	                    ...
	                    foreach( $time_slots as $tk => $tv ) {
	                        $timeslot_values = palpodelivery_common::get_timeslot_values( $tv );
	                        if( $timeslot_values[ 'lockout' ] != '' && $timeslot_values[ 'lockout' ] != '0' ) {
	                            ...
	                            if( $timeslot_values[ 'time_slot' ] == $select_time_slot_str ) {
	                                ...
	                                if( isset( $setting_value[ 'palpodelivery_lockout_time_slot' ] ) ) {
	                                    $lockout_time = $setting_value[ 'palpodelivery_lockout_time_slot' ];
	                                ...
	                                $setting_value[ 'palpodelivery_lockout_time_slot' ] = json_encode( $lockout_time_arr );
	                                update_option( $setting_key, $setting_value );
	    ...
	    if( $shipping_based_lockout == "No" ) {
	        ...
	        $delivery_dates = get_post_meta( $order_id, '_palpodelivery_shipping_delivery_dates' );
	            ...
	            foreach( $d_value as $d_k => $d_v ) {
	                if( preg_match( '/e_deliverydate/', $d_k ) ) {
	                    ...
	                    if( isset( $d_value[ "h_deliverydate_" . $address_key[ 2 ] . "_" . $address_key[ 3 ] . "_" . $address_key [ 4 ] ] ) ) {
	                        ...
	                        $delivery_date = date( palpodelivery_LOCKOUT_DATE_FORMAT, strtotime( $h_deliverydate ) );
	                        foreach ( $lockout_days_arr as $k => $v ) {
	                            $orders = $v->o;
	                            if ( $delivery_date == $v->d ) {
	                                if( $v->o == $total_quantities ) {
	        ...
	        update_option( 'palpodelivery_lockout_days', $lockout_days_jarr );
	    }
	    ...
	    if( $shipping_based_timeslot_lockout == "No" ) {
	        ...
	        $lockout_time = get_option( 'palpodelivery_lockout_time_slot' );
	        if ( $lockout_time == '' || $lockout_time == '{}' || $lockout_time == '[]' || $lockout_time == 'null' ) {
	            $lockout_time_arr = array();
	        } else {
	            $lockout_time_arr = (array) json_decode( $lockout_time );
	        ...
	        update_option( 'palpodelivery_lockout_time_slot', $lockout_time_jarr );
	    }

	    // Delete the Event from the Google Calendar
	    if( 'directly' === get_option( 'palpodelivery_calendar_sync_integration_mode' ) ) {
	    	$gcal = new palpodeliveryGcal();
	 ...
	 * @since 2.8.6
	 */
	public static function palpodelivery_admin_product_has_delivery( $product_id ) {
        $terms = get_the_terms( $product_id, 'product_cat' );
        $product_category_enabled = 'off';
        $is_enabled = 'no';
	    if( $terms == '' ) {
	        if ( has_filter( 'palpodelivery_remove_delivery_date_if_product_category_no' ) ) {
	            $is_enabled = apply_filters( 'palpodelivery_remove_delivery_date_if_product_category_no', $is_enabled );
	        }
	        if ( $is_enabled == 'yes' ) {
	        ...
	        foreach ( $terms as $term ) {
	           ...
	           $delivery_date  = get_term_meta( $categoryid, 'palpodelivery_delivery_date_for_product_category', true );
	           if ( has_filter( 'palpodelivery_remove_delivery_date_if_product_category_no' ) ) {
	               $is_enabled = apply_filters( 'palpodelivery_remove_delivery_date_if_product_category_no', $is_enabled );
	           }
	           if ( $is_enabled == 'yes' ) {
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_based_settings() {
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' ) {
	        ...
	        foreach ( $results as $key => $value ) {
	            ...
	            $shipping_method_str      = palpodelivery_common::palpodelivery_get_shipping_method_str( $shipping_settings );
	            $shipping_methods_for_categories = palpodelivery_common::palpodelivery_get_shipping_methods_for_categories( $shipping_settings );
	            $enable_delivery_date     = palpodelivery_common::palpodelivery_get_shipping_enable_delivery_date( $shipping_settings );
	            $delivery_checkout_option = palpodelivery_common::palpodelivery_get_shipping_delivery_checkout_option( $shipping_settings );
	            $date_field_mandatory     = palpodelivery_common::palpodelivery_get_shipping_date_field_mandatory( $shipping_settings );
	            $time_slots_enable        = palpodelivery_common::palpodelivery_is_shipping_timeslot_enable( $shipping_settings );
	            $timeslot_field_mandatory = palpodelivery_common::palpodelivery_get_shipping_time_field_mandatory( $shipping_settings );
	            $new_array                = palpodelivery_common::palpodelivery_get_shipping_hidden_variables( $shipping_settings , $timeslot_field_mandatory );
	            $var_time                 = palpodelivery_common::palpodelivery_get_shipping_time_settings_variable( $shipping_settings, $new_array[ 'palpodelivery_delivery_date_holidays' ], $new_array[ 'palpodelivery_lockout_days' ] );
	    		$palpodelivery_min_between_days   = palpodelivery_common::palpodelivery_get_shipping_palpodelivery_min_between_days( $shipping_settings );
                $palpodelivery_max_between_days   = palpodelivery_common::palpodelivery_get_shipping_palpodelivery_max_between_days( $shipping_settings );
                $palpodelivery_minimum_delivery_time   = palpodelivery_common::palpodelivery_get_shipping_minimum_delivery_time( $shipping_settings );
                $palpodelivery_date_field_label   = palpodelivery_common::palpodelivery_get_shipping_date_field_label( $shipping_settings );
                $palpodelivery_time_field_label   = palpodelivery_common::palpodelivery_get_shipping_time_field_label( $shipping_settings );
                ...
                $shipping_method_js_str[ $i ][ 'palpodelivery_delivery_checkout_options' ] = $delivery_checkout_option;
                $shipping_method_js_str[ $i ][ 'palpodelivery_min_between_days' ] = $palpodelivery_min_between_days;
                $shipping_method_js_str[ $i ][ 'palpodelivery_max_between_days' ] = $palpodelivery_max_between_days;
                $shipping_method_js_str[ $i ][ 'palpodelivery_minimum_delivery_time' ] = $palpodelivery_minimum_delivery_time;
                $shipping_method_js_str[ $i ][ 'palpodelivery_date_field_label' ] = __( $palpodelivery_date_field_label, 'order-delivery-date' );
                $shipping_method_js_str[ $i ][ 'palpodelivery_time_field_label' ] = __( $palpodelivery_time_field_label, 'order-delivery-date' );
                if( has_filter( 'palpodelivery_custom_delivery_settings' ) ) {
	        		$shipping_method_js_str[ $i ] = apply_filters( 'palpodelivery_custom_delivery_settings', $shipping_settings, $shipping_method_js_str[ $i ], $j );
	        	}
	            $i++;
	            $j++;
	        }
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_method_str( $shipping_settings ) {
		$shipping_method_str = '';
		if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
            if( isset( $shipping_settings[ 'shipping_methods' ] ) ) {
                $shipping_methods = $shipping_settings[ 'shipping_methods' ];
	...
	public static function palpodelivery_get_shipping_methods_for_categories( $shipping_settings ) {
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_enable_delivery_date( $shipping_settings ) {
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_delivery_checkout_option( $shipping_settings ) {
		$delivery_checkout_option = '';
		if( isset( $shipping_settings[ 'palpodelivery_delivery_checkout_options' ] ) ) {
			$delivery_checkout_option = $shipping_settings[ 'palpodelivery_delivery_checkout_options' ];
		}
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_palpodelivery_min_between_days( $shipping_settings ) {
		$palpodelivery_min_between_days = '';
		if( isset( $shipping_settings[ 'palpodelivery_min_between_days' ] ) ) {
			$palpodelivery_min_between_days = $shipping_settings[ 'palpodelivery_min_between_days' ];
		}
		return $palpodelivery_min_between_days;
	}
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_palpodelivery_max_between_days( $shipping_settings ) {
		$palpodelivery_max_between_days = '';
		if( isset( $shipping_settings[ 'palpodelivery_min_between_days' ] ) ) {
			$palpodelivery_max_between_days = $shipping_settings[ 'palpodelivery_max_between_days' ];
		}
		return $palpodelivery_max_between_days;
	}
	...
	public static function palpodelivery_get_shipping_minimum_delivery_time( $shipping_settings ) {
		$palpodelivery_minimum_delivery_time = '';
		if( isset( $shipping_settings[ 'minimum_delivery_time' ] ) ) {
			$palpodelivery_minimum_delivery_time = $shipping_settings[ 'minimum_delivery_time' ];
		}
		return $palpodelivery_minimum_delivery_time;
	}
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_date_field_label( $shipping_settings ) {
		$shipping_date_field_label = '';
		if( isset( $shipping_settings[ 'palpodelivery_shipping_based_delivery_date_field_label' ] ) && '' != $shipping_settings[ 'palpodelivery_shipping_based_delivery_date_field_label' ] ) {
			$shipping_date_field_label = $shipping_settings[ 'palpodelivery_shipping_based_delivery_date_field_label' ];
		} else {
			$shipping_date_field_label = get_option( 'palpodelivery_delivery_date_field_label' );
		}

		if( $shipping_date_field_label == '' ) {
			$shipping_date_field_label = palpodelivery_DELIVERY_DATE_FIELD_LABEL;
		}
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_time_field_label( $shipping_settings ) {
		$shipping_time_field_label = '';
		if( isset( $shipping_settings[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ] ) && '' != $shipping_settings[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ] ) {
			$shipping_time_field_label = $shipping_settings[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ];
		} else {
			$shipping_time_field_label = get_option( 'palpodelivery_delivery_timeslot_field_label' );
		}

		if( $shipping_time_field_label == '' ) {
			$shipping_time_field_label = palpodelivery_DELIVERY_TIMESLOT_FIELD_LABEL;
		}
	...
	public static function palpodelivery_get_shipping_hidden_variables( $shipping_settings, $timeslot_field_mandatory ) {
		global $palpodelivery_weekdays, $palpodelivery_shipping_days;
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
        ...
        if( isset( $delivery_type[ 'weekdays' ] ) && $delivery_type[ 'weekdays' ] == 'on' ) {
            ...
            if ( $checked == 'Yes' ) {
            	$new_array[ "palpodelivery_recurring_days" ] = $delivery_type[ 'weekdays' ];
                foreach ( $alldayskeys as $key ) {
                    $new_array[ $key ] = $alldays[$key];
                }
            } else if ( $checked == 'No' && !isset( $delivery_type[ 'specific_dates' ] ) ) {
            	$new_array[ "palpodelivery_recurring_days" ] = "";
                foreach ( $alldayskeys as $key ) {
            ...
            if( 'No' == $checked ) {
		    	$new_array[ "palpodelivery_is_all_weekdays_disabled" ] = "yes";
		    } else {
		    	$new_array[ "palpodelivery_is_all_weekdays_disabled" ] = "no";
		    }
        ...
        if( isset( $delivery_type[ 'specific_dates' ] ) && $delivery_type[ 'specific_dates' ] == 'on' ) {
            $new_array[ "palpodelivery_specific_delivery_dates" ] = $delivery_type[ 'specific_dates' ];
            ...
            if ( false != $delivery_dates_str ) {
            	$new_array[ "palpodelivery_delivery_dates" ] = $delivery_dates_str;
            } else {
            	$new_array[ "palpodelivery_delivery_dates" ] = "";
            }
        } else {
            $new_array[ "palpodelivery_delivery_dates" ] = "";
            $new_array[ "palpodelivery_specific_delivery_dates" ] = "";
        }
        ...
        if( is_admin() ) { // Holidays should be enabled for the admin
	        ...
	        $holidays_str = palpodelivery_common::palpodelivery_get_custom_holidays( $shipping_settings );
	    }

        $new_array[ "palpodelivery_delivery_date_holidays" ] = $holidays_str;

        //Lockout Days
        if( is_admin() ) { // Lockout days should not be applied for the admin
            $new_array[ "palpodelivery_lockout_days" ] = '';
        } else {
	        $lockout_days_str = palpodelivery_common::palpodelivery_get_custom_lockout_days( $shipping_settings );
	        $new_array[ "palpodelivery_lockout_days" ] = $lockout_days_str;
	    }
        ...
        if( isset( $shipping_settings[ 'time_settings' ] ) ) {
            ...
            if( isset( $time_settings[ 'from_hours' ] ) && $time_settings[ 'from_hours' ] != 0
                && isset( $time_settings[ 'to_hours' ] ) && $time_settings[ 'to_hours' ] != 0 ) {
                $from_hour_values = palpodelivery_common::palpodelivery_get_shipping_from_time( $time_settings, $shipping_settings, $holidays_str, $lockout_days_str );
                ...
                $new_array[ "palpodelivery_min_hour" ] = $from_hours;
                $new_array[ "palpodelivery_min_minute" ] = $from_mins;
                $new_array[ "palpodelivery_min_hour_set" ] = $time_settings[ 'from_hours' ];
                $new_array[ "palpodelivery_min_mins_set" ] = $time_settings[ 'from_mins' ];
                $new_array[ "palpodelivery_max_hour_set" ] = $time_settings[ 'to_hours' ];
                $new_array[ "palpodelivery_max_mins_set" ] = $time_settings[ 'to_mins' ];
                $new_array[ "palpodelivery_enable_time_slider" ] = "on";
                $new_array[ "palpodelivery_show_datepicker" ] = 'datetimepicker';
                $time_slider_enabled = 'on';
            } else {
                $new_array[ "palpodelivery_enable_time_slider" ] = "" ;
                $new_array[ "palpodelivery_show_datepicker" ] = 'datepicker';
            }
        }

		$same_day_cut_off = palpodelivery_get_highest_same_day();
        $same_day = array();
        ...
        //Nexy Day Delivery
	    $next_day_cut_off = palpodelivery_get_highest_next_day();
        $next_day = array();
		...
		if( is_array( $same_day ) && count( $same_day ) > 0 ) {
            if( isset( $same_day[ 'after_hours' ] ) && $same_day[ 'after_hours' ] == 0 && isset( $same_day[ 'after_minutes' ] ) && $same_day[ 'after_minutes' ] == 00 ) {
                $new_array[ "palpodelivery_custom_based_same_day_delivery" ] = "";
            } else {
                ...
                $new_array[ "palpodelivery_custom_based_same_day_delivery" ] = "on";
            }
        } else {
            $new_array[ "palpodelivery_custom_based_same_day_delivery" ] = "";
        }
		...
		if( is_array( $next_day ) && count( $next_day ) > 0 ) {
            if( isset( $next_day[ 'after_hours' ] ) &&
                ...
                $new_array[ "palpodelivery_custom_based_next_day_delivery" ] = "";
                ...
                $new_array[ "palpodelivery_custom_based_next_day_delivery" ] = "on";
            }
        } else {
            $new_array[ "palpodelivery_custom_based_next_day_delivery" ] = "";
        }

        $disabled_days = palpodelivery_common::palpodelivery_get_shipping_disabled_days_str( $shipping_settings, $same_day, $next_day );
        ...
        $new_array[ "palpodelivery_disabled_days_str" ]     = $disabled_days_str;
        $new_array[ "palpodelivery_disabled_weekdays_str" ] = $disabled_weekdays_str;

        //Minimum Delivery Time
        $minimum_time  = palpodelivery_get_higher_minimum_delivery_time();
		...
		$new_array[ "palpodelivery_next_day" ] = date( 'j-n-Y', strtotime($current_date." +1 day") );

        $all_specific_dates = self::palpodelivery_get_all_shipping_specific_dates( $shipping_settings );
    	...
    	if( "on" == get_option( 'palpodelivery_enable_shipping_days' ) ) {
            if( "checked" != get_option( 'palpodelivery_shipping_day_' . $current_weekday ) ) {
                $current_time = strtotime( $current_date );
            }
        } else if( !in_array( "palpodelivery_weekday_" . $current_weekday, $weekdays_to_check ) &&
        ...
        $min_date_array = palpodelivery_common::get_min_date( $delivery_time_seconds,
                ...
                if( $tv != '' ) {
					...
					if( is_array( $timeslot_values[ 'selected_days' ] ) ) {
                    	if ( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
							$min_weekday = date ( 'w', strtotime( $min_date_array[ 'min_date' ] ) );
                            $min_weekday = 'palpodelivery_weekday_' . $min_weekday . '_custom_setting';
        ...
        $new_array[ "palpodelivery_current_day" ] = $current_date;
        $new_array[ "palpodelivery_minimumOrderDays" ] = $min_date_array[ 'min_date' ];
        $new_array[ "palpodelivery_current_date_to_check" ] = $min_date_array[ 'current_date_to_check' ];

        //Number of Dates to choose
        if( isset( $shipping_settings[ 'number_of_dates' ] ) ) {
            $new_array[ "palpodelivery_number_of_dates" ] = $shipping_settings[ 'number_of_dates' ];
        }

        if ( class_exists( 'WC_Subscriptions' ) && get_option( 'palpodelivery_enable_woo_subscriptions_compatibility' ) == 'on' ) {
        	if ( class_exists( 'ws_addon_for_palpodelivery' ) ) {
        		$subscrition_var = ws_addon_for_palpodelivery::palpodelivery_check_the_subscription_period();
        	}
            if( isset( $subscrition_var[ 'palpodelivery_start_date_for_subscription' ] ) ) {
                ...
                if( isset( $shipping_settings[ 'minimum_delivery_time' ] ) ) {
                    ...
                    if( $shipping_settings[ 'minimum_delivery_time' ] != 0 && $shipping_settings[ 'minimum_delivery_time' ] != '' ) {
                        $min_date_array = palpodelivery_common::get_min_date( $delivery_time_seconds, array( 'enabled' => $time_slider_enabled, 'from_hours' => $from_hours, 'to_hours' => $time_settings[ 'to_hours' ] ), $holidays_str, $lockout_days_str, $shipping_settings, $sameday_nextday_settings );
                    }
                }
				$new_array[ "palpodelivery_start_date_for_subscription" ] = $min_date_array[ 'min_date' ];
            }
        }

		// Partially Booked dates
		$partially_booked_dates_str = palpodelivery_widget::get_partially_booked_dates( '', $shipping_settings );
	    $available_deliveries_arr = explode( '&', $partially_booked_dates_str );

	    $new_array[ "palpodelivery_partially_booked_dates" ] = $available_deliveries_arr[0];

	    if( isset( $available_deliveries_arr[1] ) ) {
	    	$new_array[ "palpodelivery_available_deliveries" ] = $available_deliveries_arr[1];
	    }

		$palpodelivery_minimum_delivery_time   = palpodelivery_common::palpodelivery_get_shipping_minimum_delivery_time( $shipping_settings );
		$new_array[ "palpodelivery_minimum_delivery_time" ] = $palpodelivery_minimum_delivery_time;
	...
	public static function palpodelivery_get_custom_holidays( $shipping_settings ) {
		$holidays_str = '';
	...
	public static function palpodelivery_get_custom_lockout_days( $shipping_settings ) {
		$lockout_days_str = '';
		if( isset( $shipping_settings[ 'time_slots' ] ) && $shipping_settings[ 'time_slots' ] != '' ) {
            $specific_dates          = array();
            $delivery_days           = array();
            		...
            		if( is_array( $timeslot_values[ 'selected_days' ] ) ) {
            			if ( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ) {
            					if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            						if( 'all' == $dval ) {
            							$all_lockout += $timeslot_values[ 'lockout' ];
            						} else {
            							if ( isset( $delivery_days[$dval][$timeslot] ) ) {
            								$previous_lockout =  $delivery_days[ $dval ][$timeslot] + $timeslot_values[ 'lockout' ];
            								$delivery_days[ $dval ][$timeslot] = $previous_lockout;
            							} else {
            								$delivery_days[ $dval ][$timeslot] = $timeslot_values[ 'lockout' ];
            							}
            						}
            					} else if ( get_option( 'palpodelivery_global_lockout_time_slots' ) != '0' && get_option( 'palpodelivery_global_lockout_time_slots' ) != '' ) {
            						if( 'all' == $dval ) {
            							$all_lockout += get_option( 'palpodelivery_global_lockout_time_slots' );
            						} else {
            							if ( isset( $delivery_days[$dval][$timeslot] ) ) {
            								$previous_lockout =  $delivery_days[ $dval ][$timeslot] + get_option( 'palpodelivery_global_lockout_time_slots' );
            								$delivery_days[ $dval ][$timeslot] = $previous_lockout;
            							} else {
            								$delivery_days[ $dval ][$timeslot] = get_option( 'palpodelivery_global_lockout_time_slots' );
            							}
            						}
            					} else {
            						$blank_lockout_time_slot = "yes";
            				...
            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ){
            					if ( isset( $specific_dates[ $dval ][$timeslot] ) ) {
            						if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            							$previous_lockout =  $specific_dates[ $dval ][$timeslot] + $timeslot_values[ 'lockout' ];
            						} else if ( get_option( 'palpodelivery_global_lockout_time_slots' ) != '0' && get_option( 'palpodelivery_global_lockout_time_slots' ) != '' ) {
            							$previous_lockout =  $specific_dates[ $dval ][$timeslot] + get_option( 'palpodelivery_global_lockout_time_slots' );
            						} else {
            							$blank_lockout_time_slot = "yes";
            						}
            						$specific_dates[ $dval ][$timeslot] = $previous_lockout;
            						...
            						if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
            							...
            							$specific_dates[ $dval ][$timeslot] = get_option( 'palpodelivery_global_lockout_time_slots' );
            						} else {
            							$blank_lockout_time_slot = "yes";
            						}
            					}
			...
			foreach( $delivery_days as $del_k => $del_v ) {
				foreach( $del_v as $time => $lockout ) {
					$delivery_days[ $del_k ][$timeslot] = $lockout + $all_lockout;
				}
			}
			$delivery_days[ 'all' ] = $all_lockout;

			if( isset( $shipping_settings[ 'palpodelivery_lockout_time_slot' ] ) ) {
				$lockout_time = $shipping_settings[ 'palpodelivery_lockout_time_slot' ];
				if ( $lockout_time == '' || $lockout_time == '{}' || $lockout_time == '[]' || $lockout_time == 'null' ) {
					$lockout_time_arr = array();
				} else {
			...
			foreach ( $lockout_time_arr as $k => $v ) {
				...
				$date_str = date( 'j-n-Y', strtotime( $v->d ) );
				if ( isset( $date[$date_str][$lockout_time_str] ) ) {
					$previous_orders =  $date[ $date_str ][$lockout_time_str] + $v->o;
					$date[ $date_str ][$lockout_time_str] = $previous_orders;
				} else {
					$date[ $date_str ][$lockout_time_str] = $v->o;
				}
			}

			$lockout_date_arr = array();

			if ( isset( $timeslot_field_mandatory ) && "" == $timeslot_field_mandatory ) {

				if ( isset( $shipping_settings[ 'date_lockout' ] ) && $shipping_settings[ 'date_lockout' ] != '0' &&  $shipping_settings[ 'date_lockout' ] != '' ) {
	                $lockout_date_arr = array();
	                if( isset( $shipping_settings[ 'palpodelivery_lockout_date' ] ) ) {
	                    $lockout_date = $shipping_settings[ 'palpodelivery_lockout_date' ];
	                    if ( $lockout_date == '' || $lockout_date == '{}' || $lockout_date == '[]' || $lockout_date == 'null' ) {
	                        $lockout_date_arr = array();
	                    } else {
	                        $lockout_date_arr = json_decode( $lockout_date );
	                    }
	                } else {
	                    $lockout_date_arr = array();
	                }
				...
				foreach ( $lockout_date_arr as $lockout_date_arr_key => $lockout_date_arr_value ) {
					$ordd_explode_date = explode("-", $lockout_date_arr_value->d );
					$date_str = $ordd_explode_date [ 1 ] . "-" . $ordd_explode_date [ 0 ] . "-" . $ordd_explode_date [ 2 ];

					if ( array_key_exists( $date_str, $date ) ) {
						/**
						 * It indicates that order is placed without time slot.
						 */
						if ( $lockout_date_arr_value->o > $date[ $date_str ] ) {
							$previous_orders =  $date[ $date_str ] + ( $lockout_date_arr_value->o - $date[ $date_str ] );
							$date[ $date_str ] = $previous_orders;
						}
					} else {
						$date[ $date_str ] = $lockout_date_arr_value->o;
					}
            ...
            foreach ( $date as $k => $v ) {
				foreach( $v as $timeslot => $lockout ) {
					$specific_date = date( 'n-j-Y', strtotime( $k ) );
					if ( is_array( $specific_dates ) && count( $specific_dates ) > 0 && isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) &&  $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] == 'on' ) {
						if ( isset( $specific_dates[ $specific_date ][$timeslot] ) && $v[$timeslot] >= $specific_dates[ $specific_date ][$timeslot] && $specific_dates[ $specific_date ][$timeslot] != '' && $blank_lockout_time_slot != "yes" ) {
							if( isset( $lockout_arr[$k] ) ) {
								array_push( $lockout_arr[$k], $timeslot );
							}else {
					...
					if ( is_array( $delivery_days ) && count( $delivery_days ) > 0 && !isset( $specific_dates[ $specific_date ] ) ) {
						$weekday = date( 'w', strtotime( $k ) );
						if ( isset( $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ][$timeslot] ) ) {
							if( $lockout >= $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ][$timeslot] && '' != $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ][$timeslot]  && '0' != $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ][$timeslot] && 'yes' != $blank_lockout_time_slot) {
								if( isset( $lockout_arr[$k] ) ) {
									array_push( $lockout_arr[$k], $timeslot );
								}else {
									$lockout_arr[$k] = array( $timeslot );
            ...
            foreach ( $lockout_arr as $k => $v ) {
				$specific_date = date( 'n-j-Y', strtotime( $k ) );
				if(  isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) &&  $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] == 'on' && isset( $specific_dates[ $specific_date ] ) && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $specific_dates[ $specific_date ] ) ) {
					$date_to_block = explode( "-", $k );
					$lockout_days_str .= '"' . intval( $date_to_block[ 1 ] ) . '-' . intval( $date_to_block[ 0 ] ) . '-' . $date_to_block[ 2 ] . '",';
					continue;
				}
				$weekday = date( 'w', strtotime( $k ) );

				if ( isset( $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ] ) && is_array( $lockout_arr[$k] ) && count( $lockout_arr[$k] ) >= count( $delivery_days[ 'palpodelivery_weekday_' . $weekday . '_custom_setting' ] )  ) {
					$date_to_block = explode( "-", $k );
            		$lockout_days_str .= '"' . intval( $date_to_block[ 1 ] ) . '-' . intval( $date_to_block[ 0 ] ) . '-' . $date_to_block[ 2 ] . '",';
				}
            }
			...
			//Check the lockout for the specific dates in custom shipping method.
				...
				foreach( $specific_days_settings as $sk => $sv ) {
					if( $sv != '' ) {
						...
						if( isset($specific_date_arr[3]) && '' != $specific_date_arr[3] ) {
							foreach ( $lockout_date_arr as $k => $v ) {
								if ( $v->d == $specific_date_arr[0] && $v->o >= $specific_date_arr[3] ) {
									$lockout_days_str .= '"' . $v->d . '",';
								}
							}
						}
			...
			//Check the lockout for the weekdays. If it is a specific date then consider the specifix dates's lockout.
			if ( isset( $shipping_settings[ 'date_lockout' ] ) && $shipping_settings[ 'date_lockout' ] != '0' &&  $shipping_settings[ 'date_lockout' ] != '' ) {
                $lockout_date_arr = array();
                if( isset( $shipping_settings[ 'palpodelivery_lockout_date' ] ) ) {
                    $lockout_date = $shipping_settings[ 'palpodelivery_lockout_date' ];
                ...
                foreach ( $lockout_date_arr as $k => $v ) {
                    ...
                    if ( $v->o >= $shipping_settings[ 'date_lockout' ] ) {
                        $lockout_days_str .= '"' . $v->d . '",';
                    }
                }
			}
	 ...
	 *
	 */
	public static function palpodelivery_get_all_shipping_specific_dates( $shipping_settings ) {
	...
	public static function palpodelivery_get_shipping_time_settings_variable( $shipping_settings, $holidays_str, $lockout_str ) {
		$var_time     = '';
		$from_hours   = '';
		$from_mins = '';
		if( isset( $shipping_settings[ 'time_settings' ] ) ) {
            ...
            if( isset( $time_settings[ 'from_hours' ] ) && $time_settings[ 'from_hours' ] != 0
                ...
                $from_hour_values = palpodelivery_common::palpodelivery_get_shipping_from_time( $time_settings, $shipping_settings, $holidays_str, $lockout_str );
                if( is_array( $from_hour_values ) && count( $from_hour_values ) ) {
                	$from_hours = $from_hour_values[ 'from_hours' ];
                	$from_mins = $from_hour_values[ 'from_minutes' ];
                }
	...
	public static function palpodelivery_get_shipping_from_time( $time_settings, $shipping_settings, $holidays_str, $lockout_str ) {
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
        ...
        $delivery_time_seconds = intval( $minimum_delivery_time ) * 60 * 60;
		...
		if( is_array( $shipping_settings ) &&
        	...
        	( ( isset( $sameday_nextday_settings[ 'next_day' ] ) &&
        		...
        		( isset( $sameday_nextday_settings[ 'next_day' ] ) &&
        			is_array( $sameday_nextday_settings[ 'next_day' ] ) &&
        			count( $sameday_nextday_settings[ 'next_day' ] ) > 0
        		)
    		)
		) {
        ...
        $min_date_array        = palpodelivery_common::get_min_date( $delivery_time_seconds,  array( 'enabled' => 'on', 'from_hours' => $time_settings[ 'from_hours' ], 'from_mins' => $time_settings[ 'from_mins' ], 'to_hours' => $time_settings[ 'to_hours' ], 'to_mins' => $time_settings[ 'to_mins' ] ), $holidays_str, $lockout_str, $shipping_settings );
	...
	public static function palpodelivery_get_shipping_date_field_mandatory( $shipping_settings ) {
	...
	public static function palpodelivery_is_shipping_timeslot_enable( $shipping_settings ) {
		$time_slots_enable = "";
		if( isset( $shipping_settings[ 'time_slots' ] ) && $shipping_settings[ 'time_slots' ] != '' ) {
	...
	public static function palpodelivery_get_shipping_time_field_mandatory( $shipping_settings ) {
	...
	public static function palpodelivery_get_shipping_disabled_days_str( $shipping_settings, $same_day, $next_day ) {
		global $palpodelivery_shipping_days, $palpodelivery_weekdays;
        ...
        if( ( isset( $same_day[ 'after_hours' ] ) && $same_day[ 'after_hours' ] == 0 && isset( $same_day[ 'after_minutes' ] ) && $same_day[ 'after_minutes' ] == 00 ) || ( !isset( $same_day[ 'after_hours' ] ) ) ) {
            ...
            if( get_option( 'palpodelivery_enable_shipping_days' ) == 'on' ) {
                ...
                if ( $cut_off_timestamp > $cut_off_with_min_time ) {
                	...
                	if( !in_array( date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time ), $holiday_date_arr ) ) {
                    	$disabled_days_str .= '"' . date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time ) . '",';
                    }
                }
            } else {
                if ( $cut_off_timestamp > $cut_off_with_min_time ) {
                ...
                if( isset( $shipping_settings[ 'delivery_type' ][ 'weekdays' ] ) &&  $shipping_settings[ 'delivery_type' ][ 'weekdays' ] == 'on'  ) {
                	...
                	foreach ( $weekdays as $s_key => $s_value ) {
	                    if( isset( $s_value[ 'enable' ] ) && $s_value[ 'enable' ] == "checked" ) {
	                        $palpodelivery_weekdays_enabled[ $s_key ] = $s_value[ 'enable' ];
	                    }
	                }
                }
            }
                ...
                if ( $current_time < $cut_off_timestamp && $cut_off_with_min_time < strtotime( $day_after ) ) {
                    ...
                    for ( $j = $current_weekday; $j <= 6; ) {
                    	...
                    	if( get_option( 'palpodelivery_enable_shipping_days' ) == 'on' ) {
                    		...
                    		if( ( is_array( $palpodelivery_weekdays_enabled ) && count( $palpodelivery_weekdays_enabled ) > 0 && !isset( $palpodelivery_weekdays_enabled[ 'palpodelivery_weekday_' . $j ] ) ) && ( ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && !in_array( date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time ), $all_specific_dates ) ) || is_array( $all_specific_dates ) && count( $all_specific_dates ) == 0 ) ) {
								$is_day_disabled = 'yes';
                        	}
                    	}
                	...
                	if ( ( isset( $same_day[ 'after_hours' ] ) &&
                    	...
                    	$disabled_weekdays_str .= '"' . date( palpodelivery_HOLIDAY_DATE_FORMAT, $current_time ) . '",';
                    }
                } else {
                	for ( $j = $next_date; $j <= 6; ) {
                    	$is_day_disabled = 'no';
                    	...
                    	if( $is_day_disabled == 'yes' ) {
                        	...
                        	if( $current_time <= $next_date_timestamp ) {
	                        	if( in_array( date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_date_timestamp ), $holiday_date_arr ) ) {
		                    		$days_disabled = "Yes";
					    	    	$next_date_timestamp = strtotime( "+1 day", $next_date_timestamp );
					    	    	$current_time = strtotime( "+1 day", $current_time );
		                        	$j = date( 'w', $current_time );
                    ...
                    if( $days_disabled == "Yes" ) {
                        $disabled_days_str .= '"' . date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_date_timestamp ) . '",';
                    } else {
                        $disabled_days_str .= '"' . date( palpodelivery_HOLIDAY_DATE_FORMAT, $next_date_timestamp ) . '",';
                    }
                }
	 ...
	 * @globals array $palpodelivery_weekdays Weekdays array
	 *
	 * @param string $palpodelivery_shipping_id Shipping method selected
	 * @param bool $is_ajax True if called from ajax, else false.
	 *
	 * @return array Common days array
	 * @since 6.2
	 */
	public static function palpodelivery_common_delivery_days_for_product_category( $palpodelivery_shipping_id, $is_ajax = false ) {
		global $post;
		...
		if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' ) {
			global $woocommerce, $wpdb, $palpodelivery_weekdays;
			$delivery_days             = array();
			...
			if( isset( $post->post_type ) && ( $post->post_type == 'shop_order' || $post->post_type == 'shop_subscription' ) ) {
				$categories                = palpodelivery_common::palpodelivery_get_cart_product_categories( $post->ID );
				$shipping_classes          = palpodelivery_common::palpodelivery_get_cart_shipping_classes( $post->ID );
			} else {
				$categories                = palpodelivery_common::palpodelivery_get_cart_product_categories( '' );
				$shipping_classes          = palpodelivery_common::palpodelivery_get_cart_shipping_classes( '' );
			}


			$shipping_settings_results = palpodelivery_common::palpodelivery_get_shipping_settings();
				...
				if( is_array( $shipping_settings_results ) && count( $shipping_settings_results ) > 0 ) {
					foreach ( $shipping_settings_results as $skey => $svalue ) {
						...
						if ( isset( $shipping_settings['enable_shipping_based_delivery'] ) && isset( $shipping_settings[ 'delivery_settings_based_on' ][0] ) && $shipping_settings[ 'delivery_settings_based_on' ][0] == 'product_categories' && isset( $shipping_settings[ 'product_categories' ] ) ) {
							...
							} else if( isset( $shipping_settings[ 'shipping_methods_for_categories' ] ) && in_array( $palpodelivery_shipping_id, $shipping_settings[ 'shipping_methods_for_categories' ] ) ) {
								if ( ( ( isset( $categories_checked[ $palpodelivery_shipping_id ] ) && !in_array( $value, $categories_checked[ $palpodelivery_shipping_id ] ) ) || !isset( $categories_checked[ $palpodelivery_shipping_id ] ) ) && in_array( $value, $shipping_settings[ 'product_categories' ] ) ) {
									$categories_checked[ $palpodelivery_shipping_id ][] = $value;
									$settings_to_compare[] = $shipping_settings;
								}
							}
						}
			...
			if( is_array( $settings_to_compare ) && count( $settings_to_compare ) > 0 ) {
			            		...
			            		if( is_array( $timeslot_values[ 'selected_days' ] ) && $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
		            				foreach( $timeslot_values[ 'selected_days' ] as $dkey => $dval ) {
		            					if( $timeslot_values[ 'lockout' ] != "" && $timeslot_values[ 'lockout' ] != "0" ) {
		            						...
		            						$date_time_lockouts[ $dval ][ $timeslot_values[ 'time_slot' ] ][] = get_option( 'palpodelivery_global_lockout_time_slots' );
		            					}
		            				}
				...
				foreach ( $settings_to_compare as $ckey => $cvalue ) {
	                ...
	                // We do store the global holidays in the cache 'palpodelivery_general_delivery_date_holidays' but it returns holidays in string, and we want it in array below.
					...
					if( isset( $cvalue[ 'time_slots' ] ) && $cvalue[ 'time_slots' ] != '' ) {
						if( isset( $cvalue[ 'palpodelivery_lockout_time_slot' ] ) ) {
							$lockout_time_arr = $cvalue[ 'palpodelivery_lockout_time_slot' ];
		                ...
		                foreach( $lockout_time_array as $lk => $lv ) {
		                	...
		                	if ( isset( $min_time_lockout[ $lv->d ] ) && $lv->o >= $min_time_lockout[ $lv->d ] && !in_array( $lockout_date, $locked_days ) ) {
		                		...
		                		if ( isset( $min_time_lockout[ 'all' ] ) && $lv->o >= $min_time_lockout[ 'all' ] && !in_array( $lockout_date, $locked_days ) ) {
		                			$locked_days[] = $lockout_date;
		                		} else if ( isset( $min_time_lockout[ 'palpodelivery_weekday_' . $dw ][ $lv->t ] ) && $lv->o >= $min_time_lockout[ 'palpodelivery_weekday_' . $dw ][ $lv->t ] && !in_array( $lockout_date, $locked_days ) ) {
									$locked_days[] = $lockout_date;
		                		}
		                	}
		                }
					} else {
						if( isset( $cvalue[ 'palpodelivery_lockout_date' ] ) ) {
		                    $lockout_date_array = $cvalue[ 'palpodelivery_lockout_date' ];
		                    if ( $lockout_date_array == '' || $lockout_date_array == '{}' || $lockout_date_array == '[]' || $lockout_date_array == 'null' ) {
		                        $lockout_date_arr = array();
		                    } else {
			...
			if( is_array( $settings_to_compare ) && count( $settings_to_compare ) > 1 ) {
				$common_delivery_days = palpodelivery_common::palpodelivery_get_common_delivery_days( $delivery_days );
				$days = array();
				...
				if( is_array( $specific_dates ) && count( $specific_dates ) > 0 ) {
					$common_delivery_dates = palpodelivery_common::palpodelivery_get_common_delivery_dates( $specific_dates, $days, $delivery_days );
				}
				$is_common = 'yes';
		...
		return array( 'palpodelivery_common_weekdays' 				  => $common_delivery_days,
					  'palpodelivery_common_specific_dates' 		  => $common_delivery_dates,
					  'palpodelivery_common_holidays' 				  => $all_holidays,
					  'palpodelivery_common_locked_days' 			  => $locked_days,
					  'palpodelivery_is_days_common' 				  => $is_days_common,
					  'palpodelivery_categories_settings_common'      => $is_common,
					  'palpodelivery_category_settings_to_load'       => $categories_to_send,
					  'palpodelivery_shipping_class_settings_to_load' => $shipping_class_to_send );
	}
	 ...
	 * @since 6.2
	 */
	public static function palpodelivery_get_common_delivery_dates( $specific_dates, $common_days, $delivery_days ) {
		$common_delivery_dates = array();
		if( is_array( $specific_dates ) && count( $specific_dates ) > 0 ) {
			$i = 0;
			$j = 1;
			if( ( is_array( $specific_dates ) && count( $specific_dates ) == 1 ) &&
				...
				foreach( $specific_dates as $sk => $sval ) {
					foreach( $sval as $ckey => $cvalue ) {
						...
						if( in_array( 'palpodelivery_weekday_' . $specific_date_weekday, $all_delivery_days ) ) {
	...
	public static function palpodelivery_get_common_delivery_days( $delivery_days ) {
		$common_delivery_days = array();
		if( is_array( $delivery_days ) && count( $delivery_days ) > 0 ) {
			$i = 0; $j = 1;
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_settings( $filter_inactive_schedules = 1 ) {
		$results = wp_cache_get( 'palpodelivery_get_shipping_settings_result' );
		if ( false === $results ) {
			global $wpdb;
			$shipping_based_settings_query = "SELECT option_value, option_name FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'palpodelivery_shipping_based_settings_%' AND option_name != 'palpodelivery_shipping_based_settings_option_key' ORDER BY option_id DESC";
			...
			if( is_array( $results ) && count( $results ) > 0 ) {
            	...
            	if ( 1 === $filter_inactive_schedules ) {
            		$fetch_settings_status_query = "SELECT option_value, option_name FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'palpodelivery_shipping_settings_status_%' ORDER BY option_id DESC";
            		$results2 = $wpdb->get_results( $fetch_settings_status_query );
            		foreach ( $results2 as $key2 => $value2 ) {
            			if ( 'inactive' === $value2->option_value ) {
            				$inactive_arr = explode( 'palpodelivery_shipping_settings_status_', $value2->option_name );
	            ...
	            foreach ( $results as $key => $value ) {
	            	...
	            	$custom_setting_id_arr = explode( 'palpodelivery_shipping_based_settings_', $value->option_name );
			...
			wp_cache_set( 'palpodelivery_get_shipping_settings_result', $results );
		}
		return $results;
	}
	 ...
	 * @since 9.6
	 */
	public static function palpodelivery_get_shipping_settings_count() {

		global $wpdb;
		$shipping_based_settings_query = "SELECT COUNT(option_name) as cnt FROM `" . $wpdb->prefix . "options` WHERE option_name LIKE 'palpodelivery_shipping_based_settings_%' AND option_name != 'palpodelivery_shipping_based_settings_option_key' ORDER BY option_id DESC";
		$results = $wpdb->get_results( $shipping_based_settings_query );
	...
	public static function palpodelivery_get_cart_product_categories( $order_id ) {
		global $woocommerce;
		$categories = array();
		if( $order_id == '' ) {
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				if( !isset( $values[ 'bundled_by' ] ) ) {
					...
					if ( '' != $terms ) {
						foreach ( $terms as $term => $val ) {
						   $terms_id[] = palpodelivery_common::get_base_product_category( $val->term_id );
						}
						// For the category ID, get the slug and create an array
						foreach( $terms_id as $term_id ) {
						    $cat_name = palpodelivery_common::ordd_get_cat_slug( $term_id );
	...
	public static function palpodelivery_get_cart_shipping_classes( $order_id ) {
		global $woocommerce;
		...
		if( $order_id == '' ) {
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				if( !isset( $values[ 'bundled_by' ] ) ) {
					...
					if ( '' != $terms ) {
						foreach ( $terms as $term => $val ) {
						   $terms_id[] = palpodelivery_common::get_base_shipping_class( $val->term_id );
						}
						// For the category ID, get the slug and create an array
						foreach( $terms_id as $term_id ) {
						    $cat_name = palpodelivery_common::ordd_get_cat_slug( $term_id );
	        ...
	        foreach( $items as $key => $value ) {
	            ...
	            if ( $terms != '' ) {
	                foreach ( $terms as $term => $val ) {
						...
						foreach( $terms_id as $term_id ) {
						    $cat_name = palpodelivery_common::ordd_get_shipping_class_slug( $term_id );
						    if( ! in_array( $cat_name, $shipping_classes ) )
	                            $shipping_classes[] = $cat_name;
						}
	                }
	 ...
	 * @since 6.2
	 */
    public static function palpodelivery_get_product_category_for_higher_minimum_delivery_time( $order_id = '' ) {

		$product_categories_to_send = wp_cache_get( 'palpodelivery_product_category_for_higher_minimum_delivery_time' );
		if ( false === $product_categories_to_send ) {
		    ...
		    wp_cache_set( 'palpodelivery_product_category_for_higher_minimum_delivery_time', $product_categories_to_send );
		}
	 ...
	 * @since 6.2
	 */
	public static function palpodelivery_get_shipping_class_for_higher_minimum_delivery_time( $order_id = '' ) {
		$shipping_class_to_send = '';
		$shipping_class = '';
	    $minimum_delivery_time = array();

	    $palpodelivery_shipping_based_delivery = get_option( 'palpodelivery_enable_shipping_based_delivery' );
	    if( '' == $order_id ) {
	    	global $woocommerce, $wpdb;

	    	if ( 'on' == $palpodelivery_shipping_based_delivery ) {
		    	foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
	            	...
	            	if ( '' != $terms ) {
	        	 		...
	        	 		foreach( $terms_id as $term_id ) {
	        	 			$shipping_class = palpodelivery_common::ordd_get_shipping_class_slug( $term_id );
	        	 		}
	           	 	}
	 ...
	 * @since 2.7.6
	 */
	public static function palpodelivery_is_delivery_enabled() {
	    global $woocommerce;
	    $delivery_enabled = 'yes';
	    if ( get_option( 'palpodelivery_no_fields_for_virtual_product' ) == 'on' && get_option( 'palpodelivery_no_fields_for_featured_product' ) == 'on' ) {
            foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
            	$product_id = $values[ 'product_id' ];
	    ...
	    } else if( get_option( 'palpodelivery_no_fields_for_virtual_product' ) == 'on' && get_option( 'palpodelivery_no_fields_for_featured_product' ) != 'on' ) {
	        foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
	            $_product = $values[ 'data' ];
	...
	public static function palpodelivery_get_address_key( $package ) {
	    $address_key = "";
	...
	public static function palpodelivery_get_total_orders_to_export() {
	    $total_orders_to_export = array();
	    $event_orders = get_option( 'palpodelivery_event_order_ids' );
	    if( $event_orders == '' || $event_orders == '{}' || $event_orders == '[]' || $event_orders == 'null' ) {
	        $event_orders = array();
	    }

	    $results = self::palpodelivery_get_all_future_orders();
	    foreach ( $results as $key => $value ) {
	...
	public static function palpodelivery_delete_exported_events() {
	    $event_orders = get_option( 'palpodelivery_event_order_ids', true );
		...
		update_option( 'palpodelivery_event_order_ids', '' );
	...
	public static function palpodelivery_get_all_future_orders() {
		global $wpdb;
	 ...
	 * @since 4.1
	 */
	public static function palpodelivery_get_total_product_quantities( $called_from = '' ) {
	    global $woocommerce;
	 ...
	 * @since 3.0
	 */
	public static function palpodelivery_get_shipping_method_enabled() {
	...
	public static function palpodelivery_get_formatted_shipping_customer_address( $user_id ) {
	    $address = '';
	 ...
	 * @since 2.7
	 */
	public static function palpodelivery_get_timeslots( $current_date, $dw, $delivery_date, $custom_settings ) {
	    global $wpdb;
            	...
            	if( is_array( $custom_settings ) && count( $custom_settings ) > 1 ) {
            		foreach( $custom_settings as $skey => $sval ) {
						...
						foreach( $time_slot_arr as $tsk => $tsv ) {
                        	if( $tsv != '' ) {
	                            $timeslot_values = palpodelivery_common::get_timeslot_values( $tsv );
	                            // Fetch all the time slots for the selected weekday or the date for all the combination of settings.
	                           	if( isset( $timeslot_values[ 'selected_days' ] ) &&
	                           		( in_array( 'palpodelivery_weekday_' . $dw . '_custom_setting', $timeslot_values[ 'selected_days' ]  ) ||
	    ...
	    if( "No" == $shipping_based ) {
	        ...
	        $existing_timeslots_str = get_option( 'palpodelivery_delivery_time_slot_log' );
	        $existing_timeslots_arr = json_decode( $existing_timeslots_str );

	        $delivery_dates_str = get_option( 'palpodelivery_delivery_dates' );
	        ...
	        if ( isset( $existing_timeslots_arr ) ) {
	            foreach ( $existing_timeslots_arr as $k => $v ){
	                ...
	                if ( gettype( json_decode( $v->dd ) ) == 'array' &&
	                	 ...
	                	 get_option( 'palpodelivery_enable_specific_delivery_dates' ) == "on" ) {
	                    ...
	                    if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 ) {
	                        $dd = json_decode( $v->dd );
	                        foreach( $dd as $dkey => $dval ) {
	                            if( $dval == "palpodelivery_weekday_" . $dw || $dval == "all" ){
	                                $arr1[ $dval ][ $key ] = $v->lockout;
	                            }
	                        }
	                    } else {
	                        if( $v->dd == "palpodelivery_weekday_" . $dw || $v->dd == "all" ){
	...
	*/
	public static function palpodelivery_compareDeepValue( $val1, $val2 ) {
	...
	public static function palpodelivery_get_timeslot_charges( $time_slot, $current_date = false ) {
		$timeslot_charges = 0;
		$shipping_based = "No";
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' ) {
	        ...
	        $shipping_settings  = array();
	       	...
	       	if( isset( $_POST[ 'palpodelivery_location' ] ) ) {
	            $location = $_POST[ 'palpodelivery_location' ];
	        }
	        ...
	        $custom_settings_to_load = palpodelivery_common::palpodelivery_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location, $lpp_location );
        ...
        if( "Yes" == $shipping_settings_exists ) {
						...
						if( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
							...
							foreach( $timeslot_values[ 'selected_days' ] as $key => $val ) {
								if( $time_slot == $time_slot_val &&
									( $val == "palpodelivery_weekday_" . $weekday . "_custom_setting"
										|| $val == "all" ) ) {
			...
			if ( isset( $existing_timeslots_arr ) ) {
	            foreach ( $existing_timeslots_arr as $k => $v ){
        			...
        			$selected_weekday = 'palpodelivery_weekday_' . $current_weekday;
		...
		if( get_option( 'palpodelivery_enable_tax_calculation_for_delivery_charges' ) == 'on' && '' !== $timeslot_charges ) {
			$palpodelivery_display_tax_in_timeslot_charges = apply_filters( 'palpodelivery_display_tax_in_timeslot_charges', 'yes' );

			$palpodelivery_calc_taxes  = get_option( 'woocommerce_calc_taxes' );
	    	if ( isset( $palpodelivery_calc_taxes) && 'yes' == $palpodelivery_calc_taxes && 'yes' == $palpodelivery_display_tax_in_timeslot_charges ) {
				$shipping_tax_class = get_option( 'woocommerce_shipping_tax_class' );
				$palpodelivery_wc_get_tax_rate = WC_Tax::get_rates();
				if( isset( $shipping_tax_class ) && '' != $shipping_tax_class && 'inherit' !== $shipping_tax_class ) {
					$palpodelivery_wc_get_tax_rate = WC_Tax::get_rates( $shipping_tax_class );
				}

				$palpodelivery_recalculated_timeslot_tax =  WC_Tax::calc_tax( $timeslot_charges, $palpodelivery_wc_get_tax_rate );

				if ( is_array( $palpodelivery_recalculated_timeslot_tax ) && count( $palpodelivery_recalculated_timeslot_tax ) > 0 ) {
					foreach ( $palpodelivery_recalculated_timeslot_tax as $tax_key => $tax_value ) {
	 ...
	 * @since 2.7
	 */
	public static function palpodelivery_get_timeslot_lockout( $shipping_settings_to_check ) {
	    global $wpdb;
	    $arr2 = array();
	    $shipping_based = "No";
		$time_format_to_show = palpodelivery_common::palpodelivery_get_time_format();
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' ) {
            ...
            if( "Yes" == $shipping_based ) {
            	...
            	foreach( $shipping_settings_to_check as $skey => $sval ) {
	                if( isset( $sval[ 'palpodelivery_lockout_time_slot' ] ) ) {
						$lockout_time = $sval[ 'palpodelivery_lockout_time_slot' ];
                ...
                if ( is_array( $lockout_time_arr ) && count( $lockout_time_arr ) > 0 ) {
                    foreach ( $lockout_time_arr as $k => $v ) {
			            ...
			            $v->t = palpodelivery_common::palpodelivery_change_time_slot_format( $v->t, $time_format_to_show );
	...
	public static function palpodelivery_get_disabled_timeslot() {
	   	$disable_days = array();

	   	$time_format_to_show = palpodelivery_common::palpodelivery_get_time_format();

	    $existing_timeslots_str = get_option( 'palpodelivery_disable_time_slot_log' );
	...
	public static function get_timeslot_to_disable( $dw, $delivery_date, $disable_days ) {
	    ...
	    if( array_key_exists( "palpodelivery_weekday_" . $dw, $disable_days ) ) {
	        foreach( $disable_days[ "palpodelivery_weekday_" . $dw ] as $dw_key => $dw_value ) {
	...
	public static function get_min_date( $delivery_time_seconds, $delivery_time_enabled, $holidays_str, $lockout_str, $shipping_settings = array(), $sameday_nextday_settings = array() ) {
	    ...
	    $is_all_past_specific_dates = palpodelivery_common::palpodelivery_get_if_past_specific_dates( $all_specific_dates );
	    $delivery_day_3 = '';
		...
		$palpodelivery_min_hours_for_holidays = 'no';
		if( has_filter( 'palpodelivery_to_calculate_minimum_hours_for_holidays' ) ) {
		    $palpodelivery_min_hours_for_holidays = apply_filters('palpodelivery_to_calculate_minimum_hours_for_holidays', $palpodelivery_min_hours_for_holidays );
		}

	    if( "on" == get_option( 'palpodelivery_enable_shipping_days' ) ) {
            if( "checked" != get_option( 'palpodelivery_shipping_day_' . $current_weekday ) || in_array( $date_to_check, $holidays ) || in_array( $date_to_check, $bookedDays ) && $is_all_shipping_days_disabled == 'No' ) {
                $current_time = strtotime( $current_date );
            }
        } else if( 'No' == $is_all_disable_weekdays &&
        			is_array( $shipping_settings ) && count( $shipping_settings ) == 0 &&
        			( ( "checked" != get_option( 'palpodelivery_weekday_' . $current_weekday ) &&
        			  ( 'on' == get_option( 'palpodelivery_enable_specific_delivery_dates' ) &&
        			  is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 &&
        			  !in_array( $date_to_check, $all_specific_dates ) ) ) ||
        			  ( "checked" != get_option( 'palpodelivery_weekday_' . $current_weekday ) &&
        			  'on' != get_option( 'palpodelivery_enable_specific_delivery_dates' ) ) ||
        			  (in_array( $date_to_check, $holidays ) && 'yes' !== $palpodelivery_min_hours_for_holidays ) ||
        			  in_array( $date_to_check, $bookedDays )
        			) ) {
            $current_time = strtotime( $current_date );
        } else if( 'No' == $is_all_disable_weekdays && is_array( $shipping_settings ) && count( $shipping_settings ) > 0 && !isset( $weekdays_to_check[ "palpodelivery_weekday_" . $current_weekday ] ) && ( !isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) || ( isset( $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] ) && 'on' == $shipping_settings[ 'delivery_type' ][ 'specific_dates' ] && is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && !in_array( $date_to_check, $all_specific_dates ) ) ) && !isset( $shipping_settings[0][ 'delivery_type' ][ 'weekdays' ] ) ) {
        	$current_time = strtotime( $current_date );
        } else if( 'Yes' == $is_all_disable_weekdays && 'on' == get_option( 'palpodelivery_enable_specific_delivery_dates' ) && is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && !in_array( $date_to_check, $all_specific_dates ) ) {
        ...
        if( $calculate_min ) {
	        ...
	        for( $i = $current_weekday; $current_time_check <= $cut_off_date_time; $i++ ) {
                if( $i >= 0 ) {
                	if( 'on' == get_option( 'palpodelivery_enable_shipping_days' ) && $is_all_shipping_days_disabled == 'No' ) {
                		$day = 'palpodelivery_shipping_day_' . $current_weekday;
                	} else {
                		$day = 'palpodelivery_weekday_' . $current_weekday;
                	}
					...
					if( is_array( $shipping_settings ) && count( $shipping_settings ) > 0 ) {
						...
						}else if ( isset( $shipping_settings[ 'delivery_type' ][ 'weekdays' ] ) && !isset( $weekdays_to_check[ "palpodelivery_weekday_" . $current_weekday ] ) ) {
							$weekday_disabled = "yes";
						}
					} else {
						if ( '' == get_option( $day ) ) {
            		...
            		if ( 'yes' == $weekday_disabled && 'No' == $is_all_disable_weekdays ) {
						...
						if( $current_time_check <= $cut_off_date_time ) {
							...
							if ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 && 'Yes' == $is_all_disable_weekdays && 'No' == $is_all_past_specific_dates ) {
								if( $palpodelivery_disable_for_holidays != 'yes' && in_array( $m . '-' . $d . '-' . $y, $holidays ) ) {
									$cut_off_date_time = strtotime( "+1 day", $cut_off_date_time );
	                        		$cut_off_timestamp = strtotime( "+1 day", $cut_off_timestamp );
	    	...
	    	if( $calculate_min ) {
		    	...
		    	for( $i = $current_weekday; ; $i++ ) {
	                if( $i >= 0 ) {
	                	...
	                	$weekday_disabled = 'no';
	                	if( is_array( $shipping_settings ) && count( $shipping_settings ) > 0 ) {
	                		if ( isset( $shipping_settings[ 'delivery_type' ][ 'weekdays' ] ) && !isset( $weekdays_to_check[ "palpodelivery_weekday_" . $current_weekday ] ) ) {
								$weekday_disabled = "yes";
	                		}
	                	} else {
	                		if ( '' == get_option( $day ) ) {
								...
								if ( is_array( $all_specific_dates ) && count( $all_specific_dates ) > 0 ) {
		                            if( !in_array( $m . "-" . $d . "-" . $y, $all_specific_dates ) && 'Yes' == $is_all_disable_weekdays && '' == $is_all_past_specific_dates ) {
		                                ...
		                                if( $delivery_day_3 != $current_time && $current_time < $delivery_day_3 ) {
											...
											if( ( $palpodelivery_disable_for_holidays != 'yes' &&  in_array( $m . '-' . $d . '-' . $y, $holidays ) ) || in_array( $m . "-" . $d . "-" . $y, $bookedDays ) ) {
												$current_time = strtotime( "+1 day", $current_time );
											} else {
												break;
											}
	...
	public static function palpodelivery_get_if_past_specific_dates( $specific_dates ) {
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
	 ...
	 * @since 2.7
	 */
	public static function palpodelivery_get_timeslot_display( $time_slot_for_order, $time_slot_for_multiple ) {
	    global $wpdb, $palpodelivery_weekdays;
	    $gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
	    ...
	    $min_hour_in_sec = palpodelivery_get_minimum_delivery_time();
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' ) {
			...
			$custom_settings = palpodelivery_common::palpodelivery_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location, $lpp_location );
        ...
        if( 'Yes' == $shipping_settings_exists ) {
			$min_hour_in_sec    = palpodelivery_get_minimum_delivery_time_custom();
        	$time_slider_enabled = '';
        	$from_hours = '';
        	$to_hours = '';
        	...
        	$min_date_array = palpodelivery_common::get_min_date( $min_hour_in_sec, array( 'enabled' => $time_slider_enabled, 'from_hours' => $from_hours, 'to_hours' => $to_hours ), $holidays_str, $lockout_str, $custom_settings_to_load, $sameday_nextday_settings );
        } else {
        	$min_date_array = palpodelivery_common::get_min_date( $min_hour_in_sec, array( 'enabled' => get_option( 'palpodelivery_enable_delivery_time' ), 'from_hours' => get_option( 'palpodelivery_delivery_from_hours' ), 'to_hours' => get_option( 'palpodelivery_delivery_to_hours' ) ), $holidays_str, $lockout_str );
        	if( 'checked' == get_option( 'palpodelivery_time_slot_asap' ) ) {
        		$asap_option = 'yes';
	    	}
		}
	    ...
	    $disable_days = palpodelivery_common::palpodelivery_get_disabled_timeslot();
	    $time_slots_to_disable = palpodelivery_common::get_timeslot_to_disable( $dw, $delivery_date, $disable_days );
        $arr1 = palpodelivery_common::palpodelivery_get_timeslots( $current_date, $dw, $delivery_date, $custom_settings );
	    $arr2 = palpodelivery_common::palpodelivery_get_timeslot_lockout( $custom_settings );

        if( "Yes" == $shipping_settings_exists ) {
        	$shipping_days = 'Yes';
            if( array_key_exists( $delivery_date, $arr1 ) ) {
                $arr3 = $arr1[ $delivery_date ];
            } else if( array_key_exists( "palpodelivery_weekday_" . $dw . "_custom_setting", $arr1 ) || array_key_exists( "all", $arr1 ) ) {
                if( array_key_exists( "palpodelivery_weekday_" . $dw . "_custom_setting", $arr1 ) ) {
                    $arr3 = array_merge( $arr3, $arr1[ "palpodelivery_weekday_" . $dw . "_custom_setting" ] );
                }
	        ...
	        if( array_key_exists( $delivery_date, $arr1 ) &&
	        	get_option( 'palpodelivery_enable_specific_delivery_dates' ) == 'on' ) {
	            $arr3 = $arr1[ $delivery_date ];
	        } else if( array_key_exists( "palpodelivery_weekday_" . $dw, $arr1 ) ||
	        		   array_key_exists( "all", $arr1 ) ) {
	            if( array_key_exists( "palpodelivery_weekday_" . $dw, $arr1 ) ) {
	                $arr3 = array_merge( $arr3, $arr1[ "palpodelivery_weekday_" . $dw ] );
	            }
	            if( array_key_exists( "all", $arr1 ) ) {
	                $arr3 = array_merge( $arr3, $arr1[ "all" ] );
	            }
        ...
        foreach( $arr3 as $key => $lockout ) {
        	if( "Yes" == $shipping_settings_exists ) {
            	...
            	if ( $lockout == '' || $lockout == '0' || $lockout == ':' ) {
					$lockout = get_option( 'palpodelivery_global_lockout_time_slots' );
					$lockout_time = isset( $custom_settings[ 'palpodelivery_lockout_time_slot' ] ) ? $custom_settings[ 'palpodelivery_lockout_time_slot' ] : '';
					$lockout_time_arr = json_decode( $lockout_time );
			...
			$key = palpodelivery_common::palpodelivery_change_time_slot_format( $key, $time_format_to_show );
			...
			$lockout = apply_filters( 'palpodelivery_overwrite_timeslot_lockout', $lockout );
            if ( $lockout != '' && $lockout != '0' && $lockout != ':' ) {
                	 ...
                	 $arr2[ $current_date ][ $key ] >= $lockout )
                {
                    ...
                    if( ( is_array( $subscription_var ) && count( $subscription_var ) > 0 &&
                    	isset( $subscription_var[ 'palpodelivery_if_renewal_subscription' ] ) &&
                    	$subscription_var[ 'palpodelivery_if_renewal_subscription' ] == 'yes' ) ) {
            ...
            if( !in_array( $key, $time_slots_to_disable ) && $time_slot_locked == 'no' ) {
				...
				if( $min_time_on_last_slot ) {
					...
					if ( version_compare( phpversion(), '5.3.0', '>' ) ) {
						...
						$difference        =   palpodelivery_common::dateTimeDiff( $date3, $date1 );
					}
	...
	public static function dateTimeDiff( $date1, $date2 ) {
	    ...
	    if ( $invert ) {
	        palpodelivery_common::palpodelivery_date_normalize( $a, $result );
	    } else {
	        palpodelivery_common::palpodelivery_date_normalize( $b, $result );
	    }
	    return $result;
	}
     ...
     * @since 5.5
     */
	public static function palpodelivery_date_range_limit( $start, $end, $adj, $a, $b, $result ) {
	    $result = (array)$result;
	    if ( $result[ $a ] < $start ) {
     ...
     * @since 5.5
     */
	public static function palpodelivery_date_range_limit_days( $base, $result ) {
	    $days_in_month_leap = array( 31, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );
	    $days_in_month = array( 31, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 );

	    palpodelivery_common::palpodelivery_date_range_limit( 1, 13, 12, "m", "y", $base );
	...
	public static function palpodelivery_date_normalize( $base, $result ) {
	    $result = palpodelivery_common::palpodelivery_date_range_limit( 0, 60, 60, "s", "i", $result );
	    $result = palpodelivery_common::palpodelivery_date_range_limit( 0, 60, 60, "i", "h", $result );
	    $result = palpodelivery_common::palpodelivery_date_range_limit( 0, 24, 24, "h", "d", $result );
	    $result = palpodelivery_common::palpodelivery_date_range_limit( 0, 12, 12, "m", "y", $result );

	    $result = palpodelivery_common::palpodelivery_date_range_limit_days( $base, $result );
	 ...
	 * @since 5.7
	 */
	public static function palpodelivery_previous_charges_label( $order_id ) {
	    ...
	    $data = get_post_meta( $order_id );
	    if( isset( $data[ '_palpodelivery_timestamp' ][ 0 ] ) && $data[ '_palpodelivery_timestamp' ][ 0 ] != '' ) {
	        $previous_charges_labels[ 'previous_order_h_date' ] = date( "d-m-Y", $data[ '_palpodelivery_timestamp' ][ 0 ] );
	        $previous_charges_labels[ 'previous_order_date_check' ] = date( "n-j-Y", $data[ '_palpodelivery_timestamp' ][ 0 ] );
	        $previous_charges_labels[ 'previous_order_weekday_check' ] = date( "w", $data[ '_palpodelivery_timestamp' ][ 0 ] );
	    }

	    if( isset( $data[ get_option( 'palpodelivery_delivery_date_field_label' ) ][ 0 ] ) && $data[ get_option( 'palpodelivery_delivery_date_field_label' ) ][ 0 ] != '' ) {
	        $previous_charges_labels[ 'previous_order_date' ] = $data[ get_option( 'palpodelivery_delivery_date_field_label' ) ][ 0 ];
	    }

	    if( isset( $data[ get_option( 'palpodelivery_delivery_timeslot_field_label' ) ][ 0 ] ) && $data[ get_option( 'palpodelivery_delivery_timeslot_field_label' ) ][ 0 ] != '' ) {
	        $previous_charges_labels[ 'previous_order_timeslot' ] = $data[ get_option( 'palpodelivery_delivery_timeslot_field_label' ) ][ 0 ];
	    }
	...
	public static function palpodelivery_update_delivery_charges( $order_id = '', $delivery_date, $time_slot ) {
	    global $wpdb;
	    ...
	    $ordd_get_day_wise_setting = get_option( 'palpodelivery_enable_day_wise_settings' );

	    $charges_arr = palpodelivery_common::palpodelivery_get_delivery_date_charges( $order_id, $delivery_date, $time_slot );
	    $order = new WC_Order( $order_id );
		...
		foreach( $results_order_item_ids as $key => $value ) {
	        ...
	        if( $value->order_item_type == 'fee' && $value->order_item_name == $charges_arr[ 'charges_label' ] && $value->order_id == $order_id ) {
	            ...
	            if( ( $charges_arr[ 'fees' ] > 0 && 'on' == $ordd_get_day_wise_setting ) || ( $charges_arr[ 'fees' ] > 0 && 'on' == get_option( 'palpodelivery_enable_shipping_based_delivery' ) ) ) {
	    ...
	    if( ( $delivery_charges == 'no' && 'on' == $ordd_get_day_wise_setting ) || ( $delivery_charges == 'no' && 'on' == get_option( 'palpodelivery_enable_shipping_based_delivery' ) ) ) {
	 ...
	 *
	 * @gloabls resource $palpodelivery_weekdays Weekdays array
	 * @globals resource $wpdb WordPress Object
	 *
	 * @param int $order_id Order ID
	 * @param string $delivery_date Selected delivery date
	...
	public static function palpodelivery_get_delivery_date_charges( $order_id = '', $delivery_date, $time_slot ) {
	    global $wpdb, $palpodelivery_weekdays;
	    if( '' == $delivery_date ) {
	        return;
	    }
	    ...
	    $previous_charges_labels = palpodelivery_common::palpodelivery_previous_charges_label( $order_id );
	    ...
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0 ) {
	        $shipping_methods_enabled = palpodelivery_common::palpodelivery_get_shipping_method_enabled();
	        foreach ( $results as $key => $value ) {
	            $var = $var_time = $delivery_dates_str = $setting_to_load_value = '';
	            $shipping_settings = get_option( $value->option_name );
	            ...
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	                ...
	                if ( isset( $_POST[ 'post_data' ] ) ) {
	                    $category_to_load_type = preg_match( '/palpodelivery_category_settings_to_load=(.*?)&/', $_POST['post_data'], $category_to_load_match );
	                    if ( isset( $category_to_load_match[ 1 ] ) ) {
	                        $setting_to_load_value = $category_to_load_match[ 1 ];
	                    }
	                } else if ( isset( $_POST[ 'palpodelivery_category_settings_to_load' ] ) ) {
	                    $setting_to_load_value = $_POST[ 'palpodelivery_category_settings_to_load' ];
	                }
	            }
	            ...
	            if( in_array( $setting_to_load_value, $custom_settings_arr ) ) {
	                ...
	                if( isset( $shipping_settings[ 'time_slots' ] ) && $shipping_settings[ 'time_slots' ] != '' ) {
	                    ...
	                    foreach( $time_slot_settings as $hk => $hv ) {
	                        if( $hv != '' ) {
	                            $timeslot_values = palpodelivery_common::get_timeslot_values( $hv );
	                            ...
	                            if( $timeslot_values[ 'delivery_days_selected' ] == 'weekdays' ) {
	                                ...
	                                foreach( $timeslot_values[ 'selected_days' ] as $key => $val ) {
	                                    if( $timeslot_selected == $timeslot_values[ 'time_slot' ] && ( $val == "palpodelivery_weekday_" . $weekday . "_custom_setting" || $val == "all" ) ) {
	                ...
	                if( isset( $delivery_type[ 'specific_dates' ] ) && $delivery_type[ 'specific_dates' ] == 'on' ) {
	                    ...
	                    foreach( $specific_days_settings as $sk => $sv ) {
	                        if( $sv != '' ) {
	                            ...
	                            if ( $date == $specific_date_arr[ 0 ] ) {
	                                if( has_filter( 'palpodelivery_add_delivery_date_fees' ) ) {
	                                    $fees = apply_filters( 'palpodelivery_add_delivery_date_fees', $delivery_date, $fees );
	                                }
	                                if ( $fees > 0 ) {
	                    ...
	                    if( $specific_fees == "No" && !in_array( $date, $delivery_dates_array ) ) {
	                        ...
	                        if( isset( $delivery_type[ 'weekdays' ] ) && $delivery_type[ 'weekdays' ] == 'on' ) {
	                            $weekdays_settings = $shipping_settings[ 'weekdays' ];
	                            foreach ( $palpodelivery_weekdays as $n => $day_name ) {
	                                if( $n == 'palpodelivery_weekday_' . $day ) {
	                                    $weekday = $weekdays_settings[ $n ];
	                                ...
	                                if( $n == 'palpodelivery_weekday_' . $previous_charges_labels[ 'previous_order_weekday_check' ] ) {
	                                    $weekday = $weekdays_settings[ $n ];
	                                    if( isset( $weekday[ 'additional_charges' ] ) && $weekday[ 'additional_charges' ] != '' && $weekday[ 'additional_charges' ] != 0 ) {
	            ...
	            if( 'No' == $shipping_based_timeslot_fees || 'No' == $shipping_based_fees ) {
	                if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	                    ...
	                    if ( isset( $_POST[ 'post_data' ] ) ) {
	                        $shipping_class_to_load_type = preg_match( '/palpodelivery_category_settings_to_load=(.*?)&/', $_POST['post_data'], $category_to_load_match );
	                        if ( isset( $shipping_class_to_load_type[ 1 ] ) ) {
	                            $shipping_class_to_load = $shipping_class_to_load_type[ 1 ];
	                        }
	                    } else if ( isset( $_POST[ 'palpodelivery_shipping_class_settings_to_load' ] ) ) {
	                        $shipping_class_to_load = $_POST[ 'palpodelivery_shipping_class_settings_to_load' ];
	                    }
	                }
	    ...
	    if( "No" == $shipping_based_timeslot_fees && '' != $time_slot ) {
            $timeslot_log_str = get_option( 'palpodelivery_delivery_time_slot_log' );
            ...
            foreach( $timeslot_log_arr as $k => $v ) {
                ...
                if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 && get_option( 'palpodelivery_enable_specific_delivery_dates' ) == "on" ) {
                    $dd = json_decode( $v->dd );
                    if ( is_array( $dd ) && count( $dd ) > 0 ) {
                        foreach( $dd as $dkey => $dval ) {
                            $specific_delivery_date = date( 'n-j-Y', strtotime( $delivery_date ) );
                ...
                if( 'No' == $specific_timeslot_fees ) {
                    ...
                    if ( gettype( json_decode( $v->dd ) ) == 'array' && count( json_decode( $v->dd ) ) > 0 ) {
                        $dd = json_decode( $v->dd );
                        foreach( $dd as $dkey => $dval ) {
                            if( $timeslot_selected == $time_slot_key && ( $dval == "palpodelivery_weekday_" . $weekday || $dval == "all" ) ) {
                                $additional_charges = $v->additional_charges;
                                $charges_arr[ 'timeslot_charges_label' ] = $v->additional_charges_label;
                                if( $additional_charges > 0 && $additional_charges != "" ) {
                                    $charges_arr[ 'time_slot_fees' ] = $additional_charges;
                        ...
                        if( $timeslot_selected == $time_slot_key && ( $v->dd == "palpodelivery_weekday_" . $weekday || $v->dd == "all" ) ) {
                            $additional_charges = $v->additional_charges;
                            $charges_arr[ 'timeslot_charges_label' ] = $v->additional_charges_label;
                            if( $additional_charges > 0 && $additional_charges != "" ) {
                                $charges_arr[ 'time_slot_fees' ] = $additional_charges;
        ...
        if( "No" == $shipping_based_fees ) {
	        ...
	        if( $specific_fees == "No" && !in_array( $date, $delivery_dates_array ) ) {
	            ...
	            $fee_var = "additional_charges_palpodelivery_weekday_" . $day;

	            $charges_arr[ 'charges_label' ] = get_option( "delivery_charges_label_palpodelivery_weekday_" . $day );
	            if( $charges_arr[ 'charges_label' ] == '' ) {
	                $charges_arr[ 'charges_label' ] = "Delivery Charges";
	            }

	            $previous_fee_var = "additional_charges_palpodelivery_weekday_" . $previous_charges_labels[ 'previous_order_weekday_check' ];
	            $charges_arr[ 'previous_charges_label' ] = get_option( "delivery_charges_label_palpodelivery_weekday_" . $previous_charges_labels[ 'previous_order_weekday_check' ] );
	        ...
	        if ( get_option( 'palpodelivery_enable_same_day_delivery' ) == 'on' ) {
	            if( !preg_match( '/[A-Za-z]/', $delivery_date ) ) {
	                if ( $current_date == $delivery_date ) {
	                    $charges_arr[ 'same_day_fees' ] = get_option( 'palpodelivery_same_day_additional_charges' );
	                }
	            }
	        }
	        ...
	        if ( get_option( 'palpodelivery_enable_next_day_delivery' ) == 'on' ) {
	            if( !preg_match( '/[A-Za-z]/', $delivery_date ) ) {
	                if ( $next_day == $delivery_date ) {
	                    $charges_arr[ 'next_day_fees' ] = get_option( 'palpodelivery_next_day_additional_charges' );
	                }
	            }
	        }
	    }
	...
	public static function palpodelivery_get_shipping_method_for_order( $order ) {
	    ...
	    foreach ( $shipping_items as $el ) {
	        ...
	        if ( isset( $el[ 'pickup_location' ] ) && '' != $el[ 'pickup_location' ] ) {
		        ...
		        $order_shipping_method_id = 'palpodelivery_pickup_location_' . $pickup_location_arr[ 'id' ];
		    }
	    }
	 ...
	 * @since 6.1
	 */
	public static function palpodelivery_get_shipping_service_name( $usps_service_id ) {
	 ...
	 * @since 7.2
	 */
	public static function palpodelivery_get_fedex_service_name( $fedex_services_id ) {
	 ...
	 * @since 8.6
	 */
	public static function palpodelivery_get_ups_service_name( $ups_services_key ) {
	 ...
	 * @since 6.5
	 */
	public static function palpodelivery_get_event_details( $order_id ) {
		$gmt = false;
        if( has_filter( 'palpodelivery_gmt_calculations' ) ) {
            $gmt = apply_filters( 'palpodelivery_gmt_calculations', '' );
        }
        ...
        $delivery_date_formatted = palpodelivery_common::palpodelivery_get_order_delivery_date( $order_id );
        $delivery_date_timestamp = get_post_meta( $order_id, '_palpodelivery_timestamp', true );
        $time_slot               = palpodelivery_common::palpodelivery_get_order_timeslot( $order_id );

        $palpodelivery = new stdClass();
        if ( $delivery_date_timestamp != '' &&
            ...
            $event_details[ '_palpodelivery_timestamp' ] = $delivery_date_timestamp;
	 ...
	 * @since 6.6
	 */
	public static function palpodelivery_get_text_block_shipping_date( $delivery_time_seconds ) {
		global $palpodelivery_date_formats, $palpodelivery_shipping_days;
        ...
        $selected_date_format = get_option( 'palpodelivery_delivery_date_format' );
        $date_format = '';
        if( isset( $palpodelivery_date_formats[ $selected_date_format ] ) ) {
        	$date_format = $palpodelivery_date_formats[ $selected_date_format ] ;
        }
	 ...
	 * @since 6.6
	 */
	public static function palpodelivery_is_delivery_calendar_enabled_for_custom_delivery() {

		$palpodelivery_is_delivery_calendar_enabled_for_custom = wp_cache_get( 'palpodelivery_is_delivery_calendar_enabled_for_custom_result' );
		if ( false === $palpodelivery_is_delivery_calendar_enabled_for_custom ) {
			global $wpdb;

		    $palpodelivery_is_delivery_calendar_enabled_for_custom = 'no';
		    $results = palpodelivery_common::palpodelivery_get_shipping_settings();
		    $shipping_settings =  array();
		    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' && is_array( $results )
		    	&& count( $results ) > 0 ) {
		        foreach ( $results as $key => $value ) {
		            $shipping_settings = get_option( $value->option_name );
		            if( isset( $shipping_settings[ 'palpodelivery_delivery_checkout_options' ] ) && $shipping_settings[ 'palpodelivery_delivery_checkout_options' ] == 'delivery_calendar' ) {
		                $palpodelivery_is_delivery_calendar_enabled_for_custom = 'yes';
		                break;
		            }
		        }
				wp_cache_set( 'palpodelivery_is_delivery_calendar_enabled_for_custom_result', $palpodelivery_is_delivery_calendar_enabled_for_custom );
			}
		}
	    return $palpodelivery_is_delivery_calendar_enabled_for_custom;
	}
	 ...
	 * @since 6.6
	 */
	public static function palpodelivery_is_text_block_enabled_for_custom_delivery() {
		global $wpdb;
	    $palpodelivery_is_text_block_enabled_for_custom = 'no';
	    $results = palpodelivery_common::palpodelivery_get_shipping_settings();
	    $shipping_settings =  array();
	    if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' && is_array( $results ) && count( $results ) > 0) {
	        foreach ( $results as $key => $value ) {
	            $shipping_settings = get_option( $value->option_name );
	            if( isset( $shipping_settings[ 'palpodelivery_delivery_checkout_options' ] ) && $shipping_settings[ 'palpodelivery_delivery_checkout_options' ] == 'text_block' ) {
	                $palpodelivery_is_text_block_enabled_for_custom = 'yes';
	                break;
	            }
	        }
	    }
	    return $palpodelivery_is_text_block_enabled_for_custom;
	}
	 ...
	 * @since 7.5
	 */
	public static function palpodelivery_get_delivery_date_field_label( $shipping_method, $product_category, $shipping_class, $location , $order_id = "" ) {
		global $wpdb;
		$delivery_date_field_label = get_option( 'palpodelivery_delivery_date_field_label' );
		if ( function_exists( 'icl_object_id' ) && $order_id != "" && is_admin() ) {
            ...
            if ( isset( $is_wpml_langauge[ 0 ] ) ) {
                ...
                $date_string_id = icl_get_string_id( $delivery_date_field_label, 'admin_texts_palpodelivery_delivery_date_field_label', 'palpodelivery_delivery_date_field_label' );
		...
		$results = palpodelivery_common::palpodelivery_get_custom_settings( $shipping_method, $shipping_class, $product_category, $location, '' );
		if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' &&
			is_array( $results ) && count( $results ) > 0 ) {
			$shipping_settings = array();
			foreach ( $results as $key => $value ) {
            	if( isset( $value[ 'palpodelivery_shipping_based_delivery_date_field_label' ] ) && $value[ 'palpodelivery_shipping_based_delivery_date_field_label' ] != '' ) {
        			$delivery_date_field_label = $value[ 'palpodelivery_shipping_based_delivery_date_field_label' ];
        		}
            }
		}
	 ...
	 * @since 7.5
	 */
	public static function palpodelivery_get_delivery_time_field_label( $shipping_method, $product_category, $shipping_class, $location, $order_id = "" ) {
		global $wpdb;
		$delivery_time_field_label = get_option( 'palpodelivery_delivery_timeslot_field_label' );
		...
		if ( function_exists( 'icl_object_id' ) && $order_id != "" && is_admin() ) {
            ...
            if ( isset( $is_wpml_langauge[ 0 ] ) ) {
                ...
                $time_string_id = icl_get_string_id( $delivery_time_field_label, 'admin_texts_palpodelivery_delivery_timeslot_field_label', 'palpodelivery_delivery_timeslot_field_label' );
		...
		if( get_option( 'palpodelivery_enable_shipping_based_delivery' ) == 'on' &&
			is_array( $results ) && count( $results ) > 0 ) {
			foreach ( $results as $key => $value ) {
            	if( isset( $value[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ] ) && $value[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ] != '' ) {
                	$delivery_time_field_label = $value[ 'palpodelivery_shipping_based_delivery_timeslot_field_label' ];
                }
            }
		}
	 ...
	 * @since 8.4
	 */
	public static function palpodelivery_get_order_location( $order_id ) {
		$location = get_post_meta( $order_id, '_palpodelivery_location', true );
		return $location;
	}
	 ...
	 * @since 8.4
	 */
	public static function palpodelivery_get_order_formatted_location( $order_id ) {
		$location = get_post_meta( $order_id, get_option( 'palpodelivery_location_field_label' ), true );
		return $location;
	}
	...
	public static function palpodelivery_get_order_shipping_method( $order_id ) {
		global $wpdb;
		$shipping_method = "";
		$shipping_label = "";
		...
		if( isset( $results ) ) {
    		...
    		if( '' != $zone_id ) {
	            if( false !== strpos( $shipping_method, 'usps' ) ) {
                    ...
                    foreach( $table_rate_shipping_classes as $tkey => $tvalue ) {
		            	if( '' == $option_settings[ 'calculation_type' ] && $tvalue->rate_label == $shipping_label ) {
        	...
        	if( false !== strpos( $shipping_method, 'ups' ) ) {
        		$shipping_method = palpodelivery_common::palpodelivery_get_ups_shipping_method( $order_id, $shipping_method, $instance_id );
        	}
        	...
        	if( false !== strpos( $shipping_method, 'betrs_shipping' ) ) {
        		$shipping_method = palpodelivery_common::palpodelivery_get_betrs_shipping_method( $order_id, $shipping_method, $instance_id );
        	}

        	//fetch shipping method of the WooCommerce Advanced Shipping plugin
        	if( false !== strpos( $shipping_method, 'advanced_shipping' ) ) {
        		$shipping_method = palpodelivery_common::palpodelivery_get_advance_shipping_method( $order_id, $shipping_method, $instance_id );
        	}
		}
	 ...
	 * @since 8.6
	 */
	public static function palpodelivery_get_ups_shipping_method( $order_id, $shipping_method, $instance_id ) {
		global $wpdb;
		$order = new WC_Order( $order_id );
			...
			foreach ( $ups_services as $ups_services_key => $ups_services_value ) {
				...
				if( '1' == $ups_services_enabled ) {
					...
					if( '' == $ups_services_name ) {
						$ups_services_name = palpodelivery_common::palpodelivery_get_ups_service_name( $ups_services_key );
					}
	 ...
	 * @since 8.6
	 */
	public static function palpodelivery_get_betrs_shipping_method( $order_id, $shipping_method, $instance_id ) {
		global $wpdb;
		$order = new WC_Order( $order_id );
	 ...
	 * @since 8.6
	 */
	public static function palpodelivery_get_advance_shipping_method( $order_id, $shipping_method, $instance_id ) {
		global $wpdb;
		$order = new WC_Order( $order_id );
	...
	public static function palpodelivery_get_time_format() {
		$time_format_to_show = 'H:i';
		$time_format = get_option( 'palpodelivery_delivery_time_format' );
		if ( $time_format == '1' ) {
	 ...
	 * @since 8.0
	 */
	public static function palpodelivery_get_custom_settings( $shipping_method, $shipping_classes, $product_categories, $location = '', $lpp_location )  {
	 	global $wpdb;
	 	...
	 	if( $lpp_location != '' ) {
	 		foreach ( $results as $key => $value ) {
	            ...
	            $shipping_settings = get_option( $value->option_name );
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) &&
	            	$shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'palpodelivery_pickup_locations' ) {
					if( in_array( $lpp_location, $shipping_settings[ 'palpodelivery_pickup_locations' ] ) ) {
						$shipping_settings_exists = 'Yes';
	 	...
	 	if( 'No' == $shipping_settings_exists ) {
	        foreach ( $results as $key => $value ) {
	            ...
	            $shipping_settings = get_option( $value->option_name );
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) &&
	            	$shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'palpodelivery_locations' ) {
					if( in_array( $location, $shipping_settings[ 'palpodelivery_locations' ] ) ) {
						$shipping_settings_exists = 'Yes';
						$custom_settings[] = $shipping_settings;
	                }
	            }
        ...
        if( 'No' == $shipping_settings_exists ) {
	        foreach ( $results as $key => $value ) {
	            ...
	            if( isset( $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] ) && $shipping_settings[ 'delivery_settings_based_on' ][ 0 ] == 'shipping_methods' ) {
	            	if( has_filter( 'palpodelivery_get_shipping_method' ) ) {
		                $shipping_methods_values = apply_filters( 'palpodelivery_get_shipping_method', $custom_settings, $_POST, $shipping_settings[ 'shipping_methods' ], $shipping_method );
		                $shipping_settings[ 'shipping_methods' ] = $shipping_methods_values[ 'shipping_methods' ];
						$shipping_method  = $shipping_methods_values[ 'shipping_method' ];
		            }
	 ...
	 * @since 8.1
	 */
	public static function palpodelivery_get_shopping_cart_hook() {
		if ( has_filter( 'palpodelivery_shopping_cart_hook' ) ) {
            $palpodelivery_shopping_cart_hook = apply_filters( 'palpodelivery_shopping_cart_hook', '' );
        } else {
        	...
        	if ( 1 == $is_amazon_plugin_active && get_option( 'palpodelivery_amazon_payments_advanced_gateway_compatibility' ) == 'on' ) {
        		$palpodelivery_shopping_cart_hook = palpodelivery_SHOPPING_CART_HOOK_AMAZON;
			} else {
                $palpodelivery_shopping_cart_hook = palpodelivery_SHOPPING_CART_HOOK;
            }
        }
        return $palpodelivery_shopping_cart_hook;
	}

	public static function palpodelivery_is_view_subscription_page() {
	 ...
	 * @since 8.5
	 */
	public static function palpodelivery_custom_sort( $a, $b ) {
		$tstamp_from_1 = 0;
		$tstamp_from_2 = 0;
		if( isset( $a->fh ) ) {
			...
			$timeslot_values_a = palpodelivery_common::get_timeslot_values( $a );
			$timeslot_values_b = palpodelivery_common::get_timeslot_values( $b );
	 ...
	 * @since 8.5
	 */
	public static function palpodelivery_sort_specific_dates( $a, $b ) {
		$tstamp_1 = 0;
		$tstamp_2 = 0;
		if( is_object( $a ) ) {
	 ...
	 * @since 9.6
	 */
	public static function palpodelivery_change_time_slot_format( $timeslot, $timeslot_format = 'H:i' ) {
	 ...
	 * @since 9.13
	 */
	public static function palpodelivery_get_individual_product_quantities( $called_from = '' ) {
	    global $woocommerce;
...
$palpodelivery_common = new palpodelivery_common();
